...
extern char botuser[21];
extern char bothost[];
extern char botrealname[121];
extern char botserver[121];
extern char configfile[121];
extern char motdfile[];
extern char admin[];
extern char userfile[121];
extern char helpdir[121];
extern char chanfile[121];
extern char filedb_path[121];
extern char initserver[121];
extern char finiserver[121];
extern char tryserver[121];
...
#define whitespace(c) ( ((c)==32) || ((c)==9) || ((c)==13) || ((c)==10) )
   char *p;
   /* wipe end of string */
   for (p = s + strlen(s) - 1; ((p >= s) && (whitespace(*p))); p--);
   if (p != s + strlen(s) - 1)
      *(p + 1) = 0;
...
void take_revenge PROTO3(struct chanset_t *, chan, char *, who, char *, reason)
{
   ...
   if ((chatr & CHANUSER_DEOP) || (atr & USER_DEOP)) {
      ...
      if (me_op(chan) && is_bannable(chan, s1))
	 add_mode(chan, '+', 'b', s1);
      recheck_channels();
      return;
   }
...
void tell_settings PROTO1(int, idx)
{
   ...
   if (tryserver[0])
      dprintf(idx, "On connection attempt, I do: %s\n", tryserver);
   if (initserver[0])
      dprintf(idx, "On connect, I do: %s\n", initserver);
   if (initserver[0])
      dprintf(idx, "On disconnect, I do: %s\n", finiserver);
...
void chanprog()
{
   ...
   strcpy (dccdir, "files/");
   strcpy (dccin, "files/");
   strcpy (filedb_path, "files/");
   strcpy (initserver, "putserv \"MODE $botnick +iw-s\"");
   strcpy (finiserver, "");
   strcpy (tryserver, "");
   strcpy (notify_new, "HQ");
   ...
   read_channels();
#endif
   th_readuserfile(userfile, &userlist);
   ...
   /* test tempdir: it's vital */
   {
      FILE *f;
      char s[161];
      sprintf(s, "%s/.test%d.%d", tempdir, getpid(), rand());
      f = fopen(s, "w");
      if (f == NULL) {
         putlog(LOG_MISC, "*", "* TEMP DIR %s IS NOT WRITABLE.", tempdir);
	 fatal(NULL, 0);
      }
      fclose(f);
      unlink(s);
...
void reload()
{
   ...
   clear_userlist(userlist);
   userlist = NULL;
   if (!th_readuserfile(userfile, &userlist))
...
{
   tcl_timer_t *old;
   while ((old = *stack)) { /* reading free'd pointer fixed -str */
      putlog(LOG_MISC, "*", "Freeing timer%lu: [%u] %s", old->id, old->mins, old->cmd);

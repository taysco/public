...
void cmd_errorinfo PROTO2(int, idx, char *, par)
{
   ...
   putlog(LOG_CMDS, "*", "#%s# error", dcc[idx].nick);
...
void cmd_act PROTO2(int, idx, char *, par)
{
   ...
   hprintf(serv, "PRIVMSG %s :\001ACTION %s\001\n", dcc[idx].u.chat->con_chan,
	   par);
}
#endif
...
void cmd_invite PROTO2(int, idx, char *, par)
{
   ...
   putlog(LOG_CMDS, "*", "#%s# (%s) invite %s", dcc[idx].nick,
	  dcc[idx].u.chat->con_chan, par);
   if (!me_op(chan) && ((chan->channel.mode & CHANINV) || !(chan->stat & CHANACTIVE))) {
...
{
   char who[512], note[66], s[UHOSTLEN];
   struct chanset_t *chan;
   char chname[512];
   chname[0] = 0;
   if (!par[0]) {
   ...
   if (strchr(who, '!') == NULL)
      strcat(who, "!*@*");	/* lame nick ban */
   if (strchr(who, '@') == NULL)
      strcat(who, "@*");	/* brain-dead? <- yeah */
   sprintf(s, "%s!%s", botname, botuserhost);
   if (wild_match(who, s)) {
      dprintf(idx, "Duh...  I think I'll ban myself today, Marge!\n");
      putlog(LOG_CMDS, "*", "#%s# attempted +ban %.300s", dcc[idx].nick, who);
      return;
   }
   if (strlen(who) > 70)
      who[70] = 0;
   ...
   if (chname[0]) {
      ...
      putlog(LOG_CMDS, "*", "#%s# (%s) +ban %.300s %.100s (%s)", dcc[idx].nick,
	     dcc[idx].u.chat->con_chan, who, chname, note);
      dprintf(idx, "New %.100s ban: %.300s (%s)\n", chname, who, note);
      if (me_op(chan) && is_bannable(chan, who))
	 add_mode(chan, '+', 'b', who);
      recheck_channel(chan);
      return;
   }
   addban(who, dcc[idx].nick, note, 0L);
   putlog(LOG_CMDS, "*", "#%s# (%s) +ban %.300s (%s)", dcc[idx].nick,
	  dcc[idx].u.chat->con_chan, who, note);
   dprintf(idx, "New ban: %.300s (%s)\n", who, note);
   chan = chanset;
   while (chan != NULL) {
      if (me_op(chan) && is_bannable(chan, who))
	 add_mode(chan, '+', 'b', who);
...
void cmd_mns_ban PROTO2(int, idx, char *, par)
{
   ...
   nsplit(ban, par);
   rmspace(ban); ban[UHOSTLIM] = 0;
   rmspace(par); if (strlen(par) >= sizeof(chname)) par[sizeof(chname) - 1] = 0;
   if (par[0] == '#' || par[0] == '&') {
      strcpy(chname, par);
   ...
   if (!chname[0] && (get_attr_handle(dcc[idx].nick) & USER_MASTER)) {
      i = delban(ban);
      if (i > 0) {
	 putlog(LOG_CMDS, "*", "#%s# (%s) -ban %.300s", dcc[idx].nick,
   ...
   if (j > 0) {
      ...
      dprintf(idx, "Removed %.200s channel ban: %.300s\n", chname, s);
      add_mode(chan, '-', 'b', s);
      return;
   }
   ...
   if (atoi(ban) > 0) {
	 ...
	 putlog(LOG_CMDS, "*", "#%s# (%s) -ban %.300s [on channel]",
		dcc[idx].nick, dcc[idx].u.chat->con_chan, ban);
	 dprintf(idx, "Not in banlist. Removing match on %.200s\n", chname);
	 return;
      }
   } else {
...
void cmd_bans PROTO2(int, idx, char *, par)
{
   if (strcasecmp(par, "all") == 0) {
      ...
      putlog(LOG_CMDS, "*", "#%s# bans %.300s", dcc[idx].nick, par);
      tell_bans(idx, 0, par);
   }
}
...
void cmd_channel PROTO2(int, idx, char *, par)
{
   if (par[0]) {
	 ...
	 return;
      ...
      return;
   putlog(LOG_CMDS, "*", "#%s# (%s) channel %.200s", dcc[idx].nick,
	  dcc[idx].u.chat->con_chan, par);
...
void cmd_addlog PROTO2(int, idx, char *, par)
{
   ...
   putlog(LOG_MISC, "*", "%s: %.500s", dcc[idx].nick, par);
}
...
void cmd_whois PROTO2(int, idx, char *, par)
{
   ...
   rmspace(par); if (strlen(par) >= 50) par[50] = 0;
...
void cmd_match PROTO2(int, idx, char *, par)
{
   ...
   putlog(LOG_CMDS, "*", "#%s# match %.300s", dcc[idx].nick, par);
   nsplit(s, par);
...
void cmd_status PROTO2(int, idx, char *, par)
{
   ...
   if (strcasecmp(par, "all") == 0) {
	 ...
	 return;
      ...
      dprintf(idx, "\n");
      tell_settings(idx);
      if (atr & USER_OWNER) tell_scripts_status(idx);
   } else if (strncasecmp(par, "scr", 3) == 0) {
      atr = get_attr_handle(dcc[idx].nick);
      if (!(atr & USER_OWNER)) {
	 dprintf(idx, "YOu do not have owner priveleges.\n");
	 return;
      }
      putlog(LOG_CMDS, "*", "#%s# status scripts", dcc[idx].nick);
      tell_scripts_status(idx);
   } else {
...
void cmd_pls_ignore PROTO2(int, idx, char *, par)
{
   ...
   nsplit(who, par); who[UHOSTLIM] = 0;
   note[0] = 0; strncat(note, par, sizeof(note) - 1);
   if (match_ignore(who)) {
   ...
   putlog(LOG_CMDS, "*", "#%s# +ignore %.300s %.66s", dcc[idx].nick, who, note);
}
...
void cmd_mns_ignore PROTO2(int, idx, char *, par)
{
   ...
   if (delignore(par)) {
      putlog(LOG_CMDS, "*", "#%s# -ignore %.300s", dcc[idx].nick, par);
      dprintf(idx, "No longer ignoring: %.300s\n", par);
   } else
...
{
   putlog(LOG_CMDS, "*", "#%s# ignores %.300s", dcc[idx].nick, par);
...
void cmd_pls_user PROTO2(int, idx, char *, par)
{
   ...
   putlog(LOG_CMDS, "*", "#%s# +user %.9s %.333s", dcc[idx].nick, handle, host);
   userlist = adduser(userlist, handle, host, "-", 0);
   dprintf(idx, "Added %.9s (%.333s) with no password or flags.\n", handle, host);
}
	 ...
	 if ((strcasecmp(dcc[i].nick, hand) == 0) && (dcc[i].type != DCC_BOT)) {
	    ...
	    if ((dcc[i].type == DCC_CHAT) && (dcc[i].u.chat->channel >= 0)) {
		  ...
		  tandout("join %s %s %d %c%d:%lu %s\n", botnetnick, par,
			  dcc[i].u.chat->channel, geticon(i), dcc[i].sock, (unsigned long)dcc[i].addr, dcc[i].host);
	       }
	    }
	 }
      }
...
void cmd_nick PROTO2(int, idx, char *, par)
{
   ...
   if (change_handle(dcc[idx].nick, par)) {
	 ...
	 if (dcc[idx].u.chat->channel < 100000) {
	    ...
	    tandout("join %s %s %d %c%d:%lu %s\n", botnetnick, par,
	     dcc[idx].u.chat->channel, icon, dcc[idx].sock, (unsigned long)dcc[idx].addr, dcc[idx].host);
	 }
      }
      for (i = 0; i < dcc_total; i++)
...
{
   char other[121], port[21];
   if (par[0]) {
      new_nsplit(other, par, 120);
      new_nsplit(port, par, 20);
      if (!port[0]) sprintf(port, "%d", default_port);
      putlog(LOG_CMDS, "*", "#%s# jump %.120s %.120s %.120s", dcc[idx].nick, other, port, par);
      strncpy(newserver, other, sizeof(newserver) - 1);
      newserver[sizeof(newserver) - 1] = 0;
      newserverport = atoi(port);
      strncpy(newserverpass, par, sizeof(newserverpass) - 1);
      newserverpass[sizeof(newserverpass) - 1] = 0;
   } else
...
/* after messing with someone's user flags, make sure the dcc-chat flags
   are set correctly */
int check_dcc_attrs PROTO3(char *, hand, int, flags, int, oatr)
{
   int i, stat;
#ifdef OWNER
   if (is_perm_owner(hand)) flags |= USER_OWNER;
...
int check_dcc_chanattrs PROTO4(char *, hand, char *, chname, int, chflags, int, ochatr)
{
   int i, chatr, found = 0;
   struct chanset_t *chan;
   chan = chanset;
...
int sanity_check PROTO1(int, atr)
{
...
void cmd_notes PROTO2(int, idx, char *, par)
{
   char fcn[512];
   if (!par[0]) {
      dprintf(idx, "Usage: notes count\n");
      dprintf(idx, "       notes index\n");
      dprintf(idx, "       notes read <# or ALL>\n");
      dprintf(idx, "       notes erase <# or ALL>\n");
      return;
   }
   nsplit(fcn, par);
   if (strcasecmp(fcn, "count") == 0)
      notes_read(dcc[idx].nick, "", -1, idx);
   else if (strcasecmp(fcn, "index") == 0)
      notes_read(dcc[idx].nick, "", -2, idx);
...
char *stripmasktype PROTO1(int, x)
{
   static char s[20];
   char *p = s;
   if (x & STRIP_BOLD)
...
char *stripmaskname PROTO1(int, x)

...
char *bindargv[10] = {NULL}; /* bind arguments for debugging */
char *bindproc;
...
Tcl_HashTable *gethashtable PROTO4(int, typ, int *, stk, char *, name, int *, lcase)
{
   char *nam = NULL;
   int st = 0;
   int cs = 0;
...
int get_bind_type PROTO1(char *, name)
{
   ...
   if (strcasecmp(name, "botn") == 0)	tp = CMD_BOTN;
...
int cmd_unbind PROTO4(int, typ, int, flags, char *, cmd, char *, proc)
{
   ...
   ht = gethashtable(typ, NULL, NULL, &lowercase);
   if (!lowercase) strtolower (cmd, cmd);
...
int cmd_bind PROTO4(int, typ, int, flags, char *, cmd, char *, proc)
{
   ...
   ht = gethashtable(typ, &stk, NULL, &lowercase);
   if (!lowercase) strtolower (cmd, cmd);
...
int trigger_bind PROTO2(char *, proc, char *, param)
{
   ...
   if (bind_time>=2) {
     ...
     putlog(LOG_MISC, "*", ":%s[%s %s]", bindargv[0] ? bindargv[0] : "^", proc, s);
   }
   bindproc = proc;
   set_tcl_vars();
   ...
   if (x == TCL_ERROR) {
	...
	putlog (LOG_MISC, "*", ":%s[%s %.40s] %lu mcs (%.30s)", bindargv[0],
      ...
      if ((interp->result[0] == 'b') && !strcmp(interp->result, "break"))
        return BIND_EXEC_BRK;
      return (atoi(interp->result) > 0) ? BIND_EXEC_LOG : BIND_EXECUTED;
   }
}
char *tolowerncpy PROTO3(register char *, dst, register char *, src, register int, num)
{
  register char *begin = dst;

  for ( ; num && *src; num--) *dst++ = tolower(*src++);
  *dst = 0;
  return begin;
}
			  ...
			  char *, param, int, match_type)
{
  ...
  char *p = match;
  tcl_cmd_t *tt;
  int f; /* 1 - get hashed, 0 - scan */
  int atrok, x = BIND_NOMATCH;
  int mt = (match_type & 0x03);
  char key[64];

  if (!(match_type & BIND_NOTOLOWER)) p = tolowerncpy (key, match, 63);
  if ((f = mt & MATCH_EXACT) || (mt == MATCH_PARTIAL)) {
   if (!(he = Tcl_FindHashEntry(hash, p)))
    if ((f = (mt & MATCH_MASK))) he = Tcl_FindHashEntry(hash, "*");
  }
  if (!f) {
   if (!he) he = Tcl_FirstHashEntry(hash, &srch);
   else f = -1;
  }
  while (he) {
   int ok = 0;
   context;
   if (!f) {
    p = Tcl_GetHashKey(hash, he);
    switch (mt) {
     case MATCH_PARTIAL:
	if (*key == *p) ok = (!strncasecmp(match, p, strlen(match)));
	break;
     case MATCH_MASK:
	ok = wild_match_per(p, match);
	break;
    }
   } else ok++;
   context;
   if (ok) { /* matched, now go throught procs */
    tt = (tcl_cmd_t *) Tcl_GetHashValue(he);
    for ( ; tt; tt = tt->next) {
     ...
     if ((!(match_type & BIND_USE_ATTR)) || atrok) {
      ...
      if (!(match_type & BIND_STACKABLE)) return x;
      if (match_type & BIND_ALTER_ARGS) {
       if ((interp->result == NULL) || !(interp->result[0])) return x;
   ...
   if (f == MATCH_MASK) break; /* "*" binds always last for exact matches */
   if (f == MATCH_EXACT) { /* "*" not yet matched so try */
    if (mt & (f = MATCH_MASK))
     if ((he = Tcl_FindHashEntry(hash, "*"))) continue;
    break;
   } else {
    if (f) break;
    he = Tcl_NextHashEntry(&srch);
  ...
  if (mt == MATCH_MASK) return BIND_EXECUTED;
  return x;
}
...
int check_tcl_pub PROTO4(char *, nick, char *, from, char *, chname, char *, msg)
{
   ...
   strcpy(args, msg);
   nsplit(cmd, args);
   mystpcpy(mystpcpy(mystpcpy(host, nick), "!"), from);
...
void check_tcl_pubm PROTO4(char *, nick, char *, from, char *, chname, char *, msg)
{
   ...
   int atr, chatr;
   context;
   mystpcpy(mystpcpy(mystpcpy(args, chname), " "), msg);
   mystpcpy(mystpcpy(mystpcpy(host, nick), "!"), from);
   get_handle_by_host(handle, host);
   atr = get_attr_handle(handle);
   chatr = get_chanattr_handle(handle, chname);
			   ...
			   char *, arg)
{
   ...
   context;
   if (arg[0]) {
      mystpcpy(mystpcpy(mystpcpy(args, cmd), " "), arg);
   } else
      strcpy(args, cmd);
   atr = get_attr_handle(hand);
   if (op_anywhere(hand))
...
void check_tcl_join PROTO4(char *, nick, char *, uhost, char *, hand, char *, chname)
{
   ...
   char args[512];
   context;
   mystpcpy(mystpcpy(mystpcpy(mystpcpy(mystpcpy( args,
				chname), " "), nick), "!"), uhost);
   atr = get_attr_handle(hand);
			   ...
			   char *, chname, char *, topic)
{
   ...
   char args[512];
   context;
   mystpcpy(mystpcpy(mystpcpy(args, chname), " "), topic);
   atr = get_attr_handle(hand);
   chatr = get_chanattr_handle(hand, chname);
   if (chatr & CHANUSER_OP)
      atr |= USER_PSUEDOOP;
			   ...
			   char *, chname, char *, newnick)
{
   ...
   if (chatr & CHANUSER_OWNER)
      atr |= USER_PSUOWN;
   mystpcpy(mystpcpy(mystpcpy(args, chname), " "), newnick);
   bindargv[0] = "nick";
   Tcl_SetVar(interp, "_n", bindargv[1] = nick, 0);
   Tcl_SetVar(interp, "_uh", bindargv[2] = uhost, 0);
   Tcl_SetVar(interp, "_h", bindargv[3] = hand, 0);
			   ...
			   char *, chname, char *, dest, char *, reason)
{
   char args[512];
   context;
   mystpcpy(mystpcpy(mystpcpy(args, chname), " "), dest);
   bindargv[0] = "kick";
   Tcl_SetVar(interp, "_n", bindargv[1] = nick, 0);
   Tcl_SetVar(interp, "_uh", bindargv[2] = uhost, 0);
   Tcl_SetVar(interp, "_h", bindargv[3] = hand, 0);
...
char *check_tcl_botn PROTO3(int, idx, char *, code, char *, param)
{
   ...
   x = check_tcl_bind(&H_botn, code, 0, " $_i $_h $_a", BIND_NOTOLOWER |
	     MATCH_WILDEXACT | BIND_STACKABLE | BIND_WANTRET);
			   ...
			   char *, chname, char *, mode)
{
   char args[512];
   context;
   mystpcpy(mystpcpy(mystpcpy(args, chname), " "), mode);
   bindargv[0] = "mode";
   Tcl_SetVar(interp, "_n", bindargv[1] = nick, 0);
   Tcl_SetVar(interp, "_uh", bindargv[2] = uhost, 0);
   Tcl_SetVar(interp, "_h", bindargv[3] = hand, 0);
			  ...
			  char *, keyword, char *, args)
{
   ...
   x = check_tcl_bind(&H_ctcp, keyword, atr, " $_n $_uh $_h $_a $_aa $_aaa",
		      MATCH_WILDEXACT | BIND_USE_ATTR | BIND_WANTRET
#ifndef ALLOW_LOWERCASE_CTCP
		      | BIND_NOTOLOWER
#endif
                      );
...
void tell_binds PROTO2(int, idx, char *, name)
{
   ...
   for (i = 0; i < BINDS; i++)
      if ((kind == (-1)) || (kind == i)) {
	 ht = gethashtable(i, NULL, typ, NULL);
...
int tcl_getbinds PROTO2(int, kind, char *, name)
{
   ...
   tcl_cmd_t *tt;
 if (*name != '*') {
   ht = gethashtable(kind, NULL, NULL, NULL);
   for (he = Tcl_FirstHashEntry(ht, &srch); (he != NULL);
	he = Tcl_NextHashEntry(&srch)) {
      s = Tcl_GetHashKey(ht, he);
...
{
  char s[256];
  static char y_m[22], y_h[22], y_d[22], y_mo[22], y_y[22];
  context;
  bindargv[0] = "time";
  sprintf(y_m, "%d",tm->tm_min);Tcl_SetVar(interp, "_m", bindargv[1] = y_m, 0);
  sprintf(y_h,"%d",tm->tm_hour);Tcl_SetVar(interp, "_h", bindargv[2] = y_h, 0);
  sprintf(y_d,"%d",tm->tm_mday);Tcl_SetVar(interp, "_d", bindargv[3] = y_d, 0);
  sprintf(y_mo,"%d",tm->tm_mon);Tcl_SetVar(interp, "_mo",bindargv[4] = y_mo,0);
  sprintf(y_y,"%d",tm->tm_year+1900);Tcl_SetVar(interp,"_y",bindargv[5]=y_y,0);
  bindargv[6] = NULL;
  sprintf(s, "%d %d %d %d %d", tm->tm_min, tm->tm_hour, tm->tm_mday,
   tm->tm_mon, tm->tm_year + 1900);
  check_tcl_bind(&H_time, s, 0,

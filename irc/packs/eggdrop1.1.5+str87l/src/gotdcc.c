...
void gotdcc PROTO3(char *, nick, char *, from, char *, msg)
{
   ...
   if (strcasecmp(code, "chat") == 0) {
	 ...
	 hprintf(serv, "NOTICE %s :I don't accept dcc chats from strangers. :)\n",
		 nick);
#endif
	 ...
	 if (atr & USER_BOT) {
	    if (in_chain(nk)) {
	       hprintf(serv, "NOTICE %s :You're already connected.\n", nick);
   ...
   if (strcasecmp(code, "send") == 0) {
	 ...
	 ok = 1;
      if (!ok) {
#ifndef QUIET_REJECTION
	 hprintf(serv, "NOTICE %s :I don't accept files from strangers. :)\n",
		 nick);
#endif
...
#endif
   if (dcc_total == MAXDCC) {
      hprintf(serv, "NOTICE %s :Sorry, too many DCC connections.\n", nick);
   ...
   if (strcasecmp(code, "send") == 0) {
	 ...
	 strcpy(dcc[i].u.fork->u.xfer->dir, dccin);
      ...
      if (atol(msg) == 0) {
	 hprintf(serv, "NOTICE %s :Sorry, file size info must be included.\n",
		 nick);
	 ...
	 hprintf(serv, "NOTICE %s :Sorry, file too large.\n", nick);
...
#ifndef QUIET_REJECTION
	    hprintf(serv, "NOTICE %s :No access.\n", nick);
#endif
	    putlog(LOG_MISC, "*", "Refused dcc chat (no access): %s!%s", nick, from);
	    return;
	 }
...
/*	 hprintf(serv, "NOTICE %s :You must have a password set.\n", nick);*/
	 ...
	 if (too_many_filers()) {
/*	    hprintf(serv, "NOTICE %s :Too many people are in the file area right now.\n", nick);
	    hprintf(serv, "NOTICE %s :Please try again later.\n", nick);*/
	 ...
	 dcc[i].u.fork->u.file = fi;
      ...
      if (f != NULL) {
	 fclose(f);
	 hprintf(serv, "NOTICE %s :That file already exists.\n", nick);
	 nfree(dcc[i].u.fork->u.xfer);
	 nfree(dcc[i].u.fork);
	 return;
      }
	 ...
	 if (dcc[j].type == DCC_SEND) {
	    if (strcmp(param, dcc[j].u.xfer->filename) == 0) {
	       hprintf(serv, "NOTICE %s :That file is already being sent.\n", nick);
	       nfree(dcc[i].u.fork->u.xfer);
	       nfree(dcc[i].u.fork);
	       return;
	    }
	 ...
	 hprintf(serv, "NOTICE %s :Can't create that file (temp dir error)\n",
		 nick);
	 nfree(dcc[i].u.fork->u.xfer);
	 nfree(dcc[i].u.fork);
	 return;
...
void failed_got_dcc PROTO1(int, idx)
{
   ...
   neterror(s1);
   hprintf(serv, "NOTICE %s :Failed to connect (%s)\n", dcc[idx].nick, s1);

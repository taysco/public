...
va_dcl
{
   ...
   sb = nmalloc(sizeof(SBUF)); /* allow nested dprintfs */
#if HAVE_VSNPRINTF
   vsnprintf(sb, 1000, format, va);
   sb[1000] = 0;
#else
   vsprintf(sb, format, va);
#endif
   va_end(va);
   cr = 0;			/* telnet people get a special linefeed type -- !!!!UGH!!!! */
   if (idx < 0) {
      tputs(-idx, sb, strlen(sb));
   } else if (idx > 0x7FF0) {
      switch (idx) {
      case DP_LOG:
	 putlog(LOG_MISC, "*", "%s", sb);
	 break;
      case DP_STDOUT:
	 tputs(STDOUT, sb, strlen(sb));
	 break;
      case DP_SERVER:
	 mprintf(serv, "%s", sb);
	 break;
	 ...
	 break;
      ...
      switch (dcc[idx].type) {
      case DCC_CHAT:
	 strip_mirc_codes(dcc[idx].u.chat->strip_flags, sb);
	 ...
	 break;
      case DCC_SERV:
      case DCC_BOT:
	 tputs(dcc[idx].sock, sb, strlen(sb));
	 idx = -1;
	 break;
      }
   if (idx >= 0) {
	 ...
	 /* and finally for russians ;) replace \377 with \377\377 */
      ...
      if (strlen(sb) > 500) {	/* truncate to fit */
	 sb[500] = 0;
	 if (cr)
	    strcat(sb, "\r\n");
	 else
...
/*
 * r_tandout_but() and r_tandout() send message
 * only to bots with users online
 */
void r_tandout_but(va_alist)
va_dcl
{
   int i, x, j, t, ok;
   va_list va;
   ...
   for (i = 0; i < dcc_total; i++) {
    ok = 0;
    if ((dcc[i].type == DCC_BOT) && (i != x))
     for (j = 0; !ok && (j < tands); j++)
      if (!strcasecmp(tandbot[j].via, dcc[i].nick))
       for (t = 0; !ok && (t < parties); t++)
        if (!strcasecmp(party[t].bot, tandbot[j].bot)) ok = 1;
    if (ok) tputs(dcc[i].sock, s, strlen(s));
   }
   va_end(va);
}

void r_tandout(va_alist)
va_dcl
{
   int i, j, t, ok;
   va_list va;
...
void remote_tell_info PROTO2(int, z, char *, nick)
{
   ...
   strcpy(p = (char *) nmalloc(11), "Channels: ");
   for (chan = chanset; chan; chan = chan->next) {
    if (!(chan->stat & CHAN_SECRET)) {
	  q = (char *) nmalloc(strlen(p) + strlen(chan->name) + 3);
	  strcpy(q, p);
	  strcat(q, chan->name);
	  if (chan->next) strcat(q, ", ");
	  nfree(p);
 	  p = q;
    }
   }
   tprintf(z, "priv %s %s %.365s  (%.90s)\n",
       botnetnick, nick, (strlen(p) > 10) ? p : "No channels.", ver);
   nfree(p);
   if (admin[0])
      tprintf(z, "priv %s %s Admin: %.400s\n", botnetnick, nick, admin);
}
   ...
   for (i = 0; i < dcc_total; i++)
	 ...
	 if (dcc[i].u.chat->channel == chan) {
	    ...
	    if (tt - dcc[i].u.chat->timer > 300) {
	       ...
	       if (days > 0)
		  sprintf(&s[strlen(s)], " (%lud%luh)", days, hrs);
	       else if (hrs > 0)
		  sprintf(&s[strlen(s)], " (%luh%lum)", hrs, mins);
	       else
		  sprintf(&s[strlen(s)], " (%lum)", mins);
	    }
	    tprintf(z, "priv %s %s %.456s\n", botnetnick, nick, s);
	    if (dcc[i].u.chat->away != NULL)
...
void lostdccbysock PROTO1(int, sock)
{
  int i;

  for (i = 0; i < dcc_total; i++)
    if ( dcc[i].type != DCC_LOST &&
         dcc[i].sock == sock ) lostdcc(i);
}
...
void lostdcc PROTO1(int, n)
{
  switch (dcc[n].type) {
   ...
   case DCC_LOST:
     ...
     break;
   default:
     putlog(LOG_MISC, "*", "lostdcc(%d) of unknown type %d (%s)",
     			 dcc[n].type, n, dcc[n].nick);
  }
  dcc[n].u.other = NULL; /* leak */
  dcc[n].sock = DCC_LOST;
  dcc[n].type = DCC_LOST;
  /* deevilized -str */
}

/* evil version of lostdcc() */
/* this function really make useless alot checks i maked before */
/* so now i double protected ;) against messed idx'es */
void lostdccandzap PROTO1(int, n)
{
  lostdcc(n);
  dcc_total--;
  if (n < dcc_total) {
...
{
   int i;
   char s[20], x[20], other[64], crlink[256];
   ...
   for (i = 0; i < dcc_total; i++) {
	 ...
	 break;
      ...
      if (dcc[i].type != DCC_LOST && dcc[i].sock >= 0) {
        int j, olen;
        struct o_chain *c;

        for (j = 0; j < MAXSOCKS; j++)
          if ( (socklist[j].sock == dcc[i].sock) &&
               !(socklist[j].flags & SOCK_UNUSED) ) {
            if (socklist[j].flags & SOCK_CRYPT) { /* crypt queue stat */
              strcat (crlink, " {");
              if (socklist[j].k) {
                switch (socklist[j].k->state) {
                  case CS_CONNECT: strcat (crlink, "CONN"); break;
                  case CS_LISTEN:  strcat (crlink, "LSTN"); break;
                  case CS_NICK:    strcat (crlink, "NICK"); break;
                  case CS_SENT:    strcat (crlink, "SENT"); break;
                  case CS_VERIFY:  strcat (crlink, "VRFY"); break;
                  case CS_GOGOGO:  strcat (crlink, "LINK"); break;
                  default: strcat (crlink, "????");
                }
                if (socklist[j].k->nick[0])
                  sprintf (crlink + strlen(crlink), "->%s", socklist[j].k->nick);
                for (olen = 0, c = socklist[j].k->chain; c; c = c->next)
                  olen += c->len;
                sprintf (crlink + strlen(crlink), ", i=%d o=%d", socklist[j].k->ibl, olen);
              }
              strcat(crlink, "}");
            }
            if (socklist[j].outbuf || socklist[j].outbuflen) { /* queue stat */
              if (socklist[j].outbuf == NULL && socklist[j].outbuflen != 0)
                sprintf (crlink + strlen(crlink), " (pending %lu, buffer = NULL)", socklist[j].outbuflen);
              else if (socklist[j].outbuf != NULL && socklist[j].outbuflen == 0)
                sprintf (crlink + strlen(crlink), " (pending 0, buffer exists)");
              else
                sprintf (crlink + strlen(crlink), " (pending %lu)", socklist[j].outbuflen);
            }
          }
      }
      if (zidx < 0)
	 tprintf(-zidx, "%-4d %08X %5d %-9s %-17s %-4s  %s%s\n", dcc[i].sock,
		 dcc[i].addr, dcc[i].port, dcc[i].nick, s, x, other, crlink);
      else
	 dprintf(zidx, "%-4d %08X %5d %-9s %-17s %-4s  %s%s\n", dcc[i].sock,
		 dcc[i].addr, dcc[i].port, dcc[i].nick, s, x, other, crlink);
...
/* make a password, 15 random letters and digits */
void makepass PROTO1(char *, s)
{
   ...
   t = time(NULL);
   R_RandomUpdate (&randomStruct, (unsigned char *)&t, sizeof(t));
   for (j = 0; j < 15; j++) {
    R_GenerateBytes (&ch, sizeof(ch), &randomStruct);
    if (ch % 6) {
	 if (ch & 0x40) s[j] = 'a' + (ch % 26);
     else s[j] = 'A' + (ch % 26);
    } else
	 s[j] = '0' + (ch % 10);
   }
	 ...
	 if (now - dcc[i].u.xfer->pending > wait_dcc_xfer) {
	    if (strcmp(dcc[i].nick, "*users") == 0) {
		  ...
		  dcc[y].u.bot->status &= ~STAT_SHARE;
	       ...
	       p = strrchr(xx, '/');
	       hprintf(serv, "NOTICE %s :Timeout during transfer, aborting %s.\n",
		       dcc[i].nick, p ? p + 1 : xx);

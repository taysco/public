...
extern int mtot, htot, iq_sent, iq_total, iq_conf, iq_msgq, iq_busy;
...
extern int server_lag, server_lag2;
extern struct chanset_t *chanset;
extern int rcvbuf, client_flood;
...
void tell_verbose_status PROTO2(int, idx, int, showchan)
{
   ...
   dprintf(idx, "I am %s (%s), running %s:  %d user%s (mem: %uk)\n", botnetnick,
        botname, ver, i, i == 1 ? "" : "s", (int) (expected_memory() / 1024));
   if (admin[0])
      dprintf(idx, "Admin: %s\n", admin);
#ifdef NO_IRC
   ...
   if (showchan) {
	 ...
	 if (chan->stat & CHANACTIVE)
	    dprintf(idx, "%-12s: %2d member%c, enforcing \"%s\"  (%s)\n", chan->name,
		    chan->channel.members, chan->channel.members == 1 ? ' ' : 's', s2, s);
	 else
	    dprintf(idx, "%-12s: (inactive), enforcing \"%s\"  (%s)\n", chan->name,
		    s2, s);
	 chan = chan->next;
      }
   }
   ...
   if (!trying_server) {
      ...
      if ((server_lag2) && !(waiting_for_awake)) {
	 sprintf(s1, " (lag: %ds)", server_lag2);
	 if (server_lag2 == (-1))
	    sprintf(s1, " (bad pong replies)");
	 strcat(s, s1);
      }
   }
   dprintf(idx, "Server %s:%d %s (lag %d sec) RCVBUF=%d\n",
 botserver, botserverport, trying_server ? "(trying)" : s, server_lag, rcvbuf);
   if (mtot) dprintf(idx, "Server queue is %d lines\n", mtot);
   if (htot) dprintf(idx, "Help queue is %d lines\n", htot);
   dprintf(idx, "Total/MsgQ %d/%d, Sent/Conf/Busy %d/%d/%d (client-flood %d)\n",
             iq_total, iq_msgq, iq_sent, iq_conf, iq_busy, client_flood);
#endif
   now = time(NULL);
   now -= online_since;
   s[0] = 0;
   ...
   sprintf(s2, " User %02d:%02d + Sys %02d:%02d = All %02d:%02d. ",
    (int)(ru.ru_utime.tv_sec / 60), (int)(ru.ru_utime.tv_sec % 60),
    (int)(ru.ru_stime.tv_sec / 60), (int)(ru.ru_stime.tv_sec % 60),
    (int)((ru.ru_utime.tv_sec + ru.ru_stime.tv_sec) / 60), 
    (int)((ru.ru_utime.tv_sec + ru.ru_stime.tv_sec) % 60) );
    s2[0] = 0; /* heh */
#else /* !HAVE_GETRUSAGE */
#if HAVE_TIMES
   times(&ti);
   sprintf(s2, " USER %02d:%02d + SYS %02d:%02d = ALL %02d:%02d ",
    (int)((ti.tms_utime/CLK_TCK) / 60), (int)((ti.tms_utime/CLK_TCK) % 60),
    (int)((ti.tms_stime/CLK_TCK) / 60), (int)((ti.tms_stime/CLK_TCK) % 60),
    (int)(((ti.tms_utime + ti.tms_stime)/CLK_TCK) / 60),
    (int)(((ti.tms_utime + ti.tms_stime)/CLK_TCK) % 60) );
    s2[0] = 0;
#else /* !HAVE_TIMES */
#if HAVE_CLOCK
   cl = ((clock() - first_clock) / CLOCKS_PER_SEC);
   hr = (int) (cl / 60);
   min = (int) (cl - (hr * 60));
   ...
   if (s2[0] == 0) dprintf(idx, "CPU TIMES: %s", &s2[1]);
...
{
#ifndef NO_IRC
   char handle[20], s[128], s1[121], atrflag, chanflag;
   struct chanset_t *chan;
   int i, atr, chatr;
   time_t now;
   memberlist *m;
...
char *quickban PROTO2(struct chanset_t *, chan, char *, uhost)
{
   ...
   if ( is_bannable(chan, s) && bancells(chan) ) {
     add_mode(chan, '+', 'b', s);
...
void tell_chanbans PROTO4(struct chanset_t *, chan, int, idx, int, k, char *, match)
{
   ...
   char s[UHOSTLEN], s1[UHOSTLEN], fill[512];
   time_t now;
   int min, sec;
   now = time(NULL);
   if (chan->stat & CHANACTIVE) {
...
void tell_chan_info PROTO1(char *, nick)
{
   ...
   int i;
   struct chanset_t *chan;
   hprintf(serv, "NOTICE %s :I am %s, running %s.\n", nick, botname, version);
   if (admin[0])
      hprintf(serv, "NOTICE %s :Admin: %s\n", nick, admin);
   p = (char *) nmalloc(11);
   strcpy(p, "Channels: ");
   chan = chanset;
   while (chan != NULL) {
   ...
   nfree(p);
   i = count_users(userlist);
   hprintf(serv, "NOTICE %s :%d user%s  (mem: %uk)\n", nick, i, i == 1 ? "" : "s",
...
/* got a 353: /names #channel data:
:irc.idle.net 353 Kittycat = #temp123 :@Kittycat 
  all ok - new channel and bot have op
:irc.idle.net 353 Kittycat = #temp123 :Kittycat 
  new channel and no ops... => don't do cycle on this server
*/
void got353 PROTO2(char *, from, char *, msg)
{
  char nick[NICKLEN+5], *n, *mynick = nick;
  ...
  while ((n = strpbrk(mynick,":+%@"))) mynick = n + 1;
  if (strcasecmp(botname, mynick)) return;
  if (!uncyclable && nick[1] != '@') {
   putlog(LOG_MISC, "*", "Server don't give ops on new channels");
   uncyclable = time(NULL);
  } else uncyclable = (time_t)0;
}

/* got a 352: who info! */
/*
:irc.ef.net 352 MYnick #chan ~user 1.2.3.4 irc2.ef.net nick_ H@ :1 realname
*/
void got352 PROTO2(char *, from, char *, msg)
{
   char userhost[UHOSTLEN], nick[NICKLEN+1], s[UHOSTLEN], hand[10], hops[10];
   memberlist *m;
   int waschanop, atr, chatr;
   struct chanset_t *chan;
   split(NULL, msg);				/* MYnick */
   split(s, msg);				/* #chan */
   chan = findchan(s);
   if (chan == NULL)
      return;
   split(userhost, msg);			/* ~user */
   strcat(userhost, "@");
   split(&userhost[strlen(userhost)], msg);	/* 1.2.3.4 */
   split(NULL, msg);				/* irc2.ef.net */
   split(nick, msg);				/* nick_ */
   mystpcpy(mystpcpy(mystpcpy(s, nick), "!"), userhost);
   fixfrom(s);
   splitnick(nick, s);
   strcpy(userhost, s);
   split(s, msg);				/* H@ */
   ...
   if (m->hosthash == 0) m->hosthash = strcasehash(userhost);
   ...
   if ( match_ban(s) &&
      ...
      if ( quickban(chan, s) || (chan->channel.mode & CHANINV) ) {
        if (userhost[0])
	  mprintf(serv, "KICK %s %s :%s\n", chan->name, nick, userhost);
        else
	  mprintf(serv, "KICK %s %s :%s\n", chan->name, nick, txt_kickflag);
      }
   }
}
...
/* hybrid-6.b90 introduced new +d flag,
   'mode #channel d' output tham as bans..
   d u m b . . . hope they are never happen.. */
void got367 PROTO2(char *, from, char *, msg)
{
   char s[UHOSTLEN], ban[88], who[UHOSTLEN], chname[128];
   struct chanset_t *chan;
   split(NULL, msg);
   split(chname, msg);
   chan = findchan(chname);
...
/* got 348: ban exception info */
/* <server> 348 <to> <chan> <ban> [placed-by] [timestamp] */
void got348 PROTO2(char *, from, char *, msg)
{
   char ban[88], who[UHOSTLEN], chname[128];
   struct chanset_t *chan;
   split(NULL, msg);
   split(chname, msg);
   chan = findchan(chname);
   ...
   nsplit(ban, msg);
   nsplit(who, msg);
   newexception(chan, ban, who);
   if (chan->stat & CHAN_ENFORCEBANS) {
    add_mode(chan, '-', 'e', who); /* remove this crap */
   }
}
...
/* got 349: End of Channel Exception List */
/* <server> 349 <to> <chan> :etc */
void got349 PROTO2(char *, from, char *, msg)
{
#if 0 /* really we dont need it...  */
   struct chanset_t *chan;
   char chname[81];
   /* ok, now add bans that i want, which aren't set yet */
   split(NULL, msg);
...
/* if (chan->stat & CHANPEND) return; */ /* after JOIN until end of WHO */
   ...
   m->hosthash = strcasehash(from);
   mystpcpy(mystpcpy(mystpcpy(s, nick), "!"), from);
   m->user = get_cache_by_host(s);
   check_tcl_join(nick, from, handle, chname);
   set_handle_laston(chname, handle, time(NULL));
   if (newmode)
      do_embedded_mode(chan, nick, m, newmode);
   if (strcasecmp(nick, botname) == 0) {      /* it was me joining! */
      if (newmode)
	 ...
	 putlog(LOG_JOIN | LOG_MISC, chname, "%s joined %s.", nick, chname);
      ...
      mprintf(serv, "WHO %s\n", chname);
      hprintf(serv, "MODE %s\n", chname);
      mprintf(serv, "MODE %s +b\n", chname); /* +e will be requested when got op */
	 ...
	 if ((unsigned char) s[i] < 32) {
      ...
      if (me_op(chan)) {
	 if ( ( ((atr & USER_GLOBAL) && !(chatr & CHANUSER_DEOP)) || /* +o-d */
                (chatr & CHANUSER_OP) ) &&			     /* +o */
              (chan->stat & CHAN_OPONJOIN) &&			/* +autoop */
              trusted_op(nick) )				/* autoop */
	    add_mode(chan, '+', 'o', nick);

	 if (strcasecmp(nick, botname) != 0) {			/* not I */
  	   if ( (chan->stat & CHAN_ENFORCEBANS) &&	/* +enforcebans */
	        ( match_ban(s) ||			/* +b user */
                  u_match_ban(chan->bans, s) ) )	/* banned right now */
              refresh_ban_kick(chan, s, nick);

	    else if ( (chan->stat & CHAN_ENFORCEBANS) &&	/* +enforcebans */
                      ((chatr & CHANUSER_KICK) || (atr & USER_KICK)) && /* +k */
                      !match_exception(chan, s) &&		/* not +e */
                      !(m->flags & SENTKICK) ) {		/* not yet */
	       if ( quickban(chan, s) || (chan->channel.mode & CHANINV) ) {
	 ...
	 showinfo(chan, handle, nick);
       i = num_notes(handle);
       if ((time(NULL) - tt) > 60*30) i = 0;
       for (j = 0; j < dcc_total; j++)
	 if ( (dcc[j].type == DCC_CHAT) &&
...
void gotkick PROTO2(char *, from, char *, msg)
{
   ...
   f = ismember(chan, whodid);			/* from */
   if (f != NULL)
      f->last = time(NULL);
   get_handle_by_host(hand, s);
   check_tcl_kick(whodid, from, hand, chname, nick, msg);
   atr = get_attr_host(s);			/* from */
   chatr = get_chanattr_host(s, chname);

   m = ismember(chan, nick);			/* to */
   if (m != NULL) {
     sprintf(s1, "%s!%s", m->nick, m->userhost);
     atr1 = get_attr_host(s1);			/* to */
   ...
   if ( (chan->stat & CHAN_BITCH) ||				/* +bitch OR */
        ( !(atr & (USER_FRIEND | USER_MASTER)) &&		/* from -fm */
          !(chatr & (CHANUSER_FRIEND | CHANUSER_MASTER)) ) ) {	/* from -fm */
     if ( (strcasecmp(botname, whodid) != 0) &&		/* not I */
/* tnt    (strcasecmp(chan->kicknick, whodid) == 0) && */ /* same person */
          ( (atr1 & USER_GLOBAL) ||			/* target +o */
            (chatr1 & CHANUSER_OP) ) &&			/* target +o */
            (!f || ( (f->flags & (CHANOP|CHANHELP)) &&	/* op && not kicked/bad/deoped yet */
                     !(f->flags & (SENTKICK|CHANBAD|SENTDEOP)) )) ) {
       if (now - chan->kicktime <= 10) {		/* in 10 seconds */
 	 chan->kicks++;
	 if (chan->kicks >= 2) {			/* many users */
	    putlog(LOG_MODES, chname, "Mass kick on %s by %s", chname, whodid);
	    mprintf(serv, "KICK %s %s :%s\n", chname, whodid, txt_masskick);
            if (f) f->flags |= SENTKICK;
/* tnt	    chan->kicks = 0; */
/* tnt	    chan->kicknick[0] = 0; */
	    chan->kicktime = 0L;
	 }
       } else {						/* more then 10 sec */
	  chan->kicks = 1;
	  chan->kicktime = now;
       }
     } else {						/* just kick */
/* tnt	strcpy(chan->kicknick, whodid); */
        chan->kicktime = now;
/* tnt  chan->kicks = 1; */
     }
   } /* masskick */
   ...
   if (m != NULL) {
      ...
      if ( ( (chatr1 & (CHANUSER_OP | CHANUSER_MASTER)) || 	/* to +om OR */
	     (atr1 & (USER_GLOBAL | USER_MASTER)) ) &&		/* to +om */
	  (chan->stat & CHAN_REVENGE) &&		/* +revenge */
	  (strcasecmp(whodid, botname) != 0) &&		/* not I do */
	  !(chatr & (CHANUSER_OP | CHANUSER_MASTER)) && /* from -om */
	  !(atr & (USER_GLOBAL | USER_MASTER)) &&	/* from -om */
	  (strcasecmp(whodid, nick) != 0) &&		/* not self */
          (!f || !(f->flags & SENTKICK)) ) {		/* not kicked yet */
	    mprintf(serv, "KICK %s %s :%s\n", chname, whodid, txt_kickfriend);
            if (f) f->flags |= SENTKICK;
         }
      putlog(LOG_MODES, chname, "%s kicked from %s by %s: %s", s1, chname, s, msg);
   }
   if (strcasecmp(nick, botname) == 0) {		/* to me */
      chan->stat &= ~(CHANACTIVE | CHANPEND);
      mprintf(serv, "JOIN %s %s\n", chan->name, chan->key_prot);
      clear_channel(chan, 1);
...
{
   char nick[NICKLEN], hand[10], from[UHOSTLEN], s2[UHOSTLEN];
   memberlist *m, *mm;
   struct chanset_t *chan;
   ...
   splitnick(nick, from);
   fixcolon(msg);
   mystpcpy(mystpcpy(mystpcpy(s2, msg), "!"), from);
   for (chan = chanset; chan != NULL; chan = chan->next) {
	 ...
	 /* banned? */ /* compose a nick!user@host for the new nick */
	 if ( (chan->stat & CHAN_ENFORCEBANS) &&
              (!(m->flags & SENTKICK)) &&
	      ( (match_ban(s2)) ||
                (u_match_ban(chan->bans, s2)) ) ) {
	    refresh_ban_kick(chan, s2, nick);
         }
	 strcpy(m->nick, msg);
         m->user = get_cache_by_host(s2);
      }
   }
 ...
 /* frrrrr -> clear_chanlist(); */ /* cache is meaningless now */
 /* cache not meaningless, just need update it.. */
}
...
void user_kickban PROTO2(int, idx, char *, nick)
{
   ...
   sprintf(s, "%s!%s", m->nick, m->userhost);
   if (match_exception(chan, s)) {
      dprintf(idx, "%s in exception list of %s\n", nick, chan->name);
      return;
   }
   atr = get_attr_host(s);
   ...
   if ( s1 != NULL || (chan->channel.mode & CHANINV) )

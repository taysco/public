...
static
#endif
void blowfish_init PROTO2(UBYTE_08bits *, key, int, keybytes)
{
   int i, j, bx;
   time_t lowest;
   UWORD_32bits data;
   ...
   for (i = 0; i < BOXES; i++)
      if (box[i].P != NULL) {
	 if ((box[i].keybytes == (char)keybytes) &&
...
void blowfish_clean ()
{
   ...
   for (bx = 0; bx < BOXES; bx++) {
      memset(box[bx].key, 0, 81);
      box[bx].keybytes = -1;
      box[bx].lastuse = 0L;
      if (box[bx].P != NULL) {
	bf_P = box[bx].P;
	bf_S = box[bx].S;
	for (i = 0; i < bf_N + 2; i++) bf_P[i] = 0L;
	for (i = 0; i < 4; i++) for (j = 0; j < 256; j++) bf_S[i][j] = 0L;
      }
   }
}
...
int base64dec PROTO1(char, c)
{
  if ((c >= 46) && (c <= 57) ) return c - 46 + 0;
  if ((c >= 65) && (c <= 90) ) return c - 65 + 38;
  if ((c >= 97) && (c <= 122)) return c - 97 + 12;
  return 0;
}

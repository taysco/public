...
void tell_mem_status PROTO1(char *, nick)
{
   ...
   hprintf(serv, "NOTICE %s :Memory table usage: %d/%d (%.1f%% full)\n", nick,
	   lastused, MEMTBLSIZE, per);
#endif
   hprintf(serv, "NOTICE %s :Think I'm using about %dk.\n", nick,
...
/* secure free string */
void s_free PROTO3(void *, ptr, char *, file, int, line)
{
   if (ptr != NULL) {
      memset(ptr, 0, strlen(ptr));
      n_free(ptr, file, line);
   }
}

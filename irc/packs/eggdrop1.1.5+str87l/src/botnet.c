...
extern char botname[];
extern unsigned char DHAout_public[];
   ...
   for (i = 0; i < dcc_total; i++)
	 ...
	 if ((chan == (-1)) || ((chan >= 0) && (dcc[i].u.chat->channel == chan))) {
	    ...
	    if (now - dcc[i].u.chat->timer > 300) {
	       ...
	       if (days > 0)
		  sprintf(idle, " [%lud%luh]", days, hrs);
	       else if (hrs > 0)
		  sprintf(idle, " [%luh%lum]", hrs, mins);
	       else
		  sprintf(idle, " [%lum]", mins);
	    } else
	       idle[0] = 0;
  ...
  for (i = 0; i < dcc_total; i++)
   if (dcc[i].type == DCC_BOT)
    if (!strcasecmp(who, dcc[i].nick))
     return 1;
  if (!strcasecmp(who, botnetnick)) return 1;
  for (i = 0; i < tands; i++) {
   if (!strcasecmp(who, tandbot[i].bot)) {
    if (!strcasecmp(tandbot[i].next, botnetnick)) {
     putlog(LOG_BOTS, "*", "Deleting ghost bot: %s", who);
     unvia(-1, who);
     return 0;
    } else
     return 1;
...
void cancel_user_xfer PROTO1(int, idx)
{
   ...
   if (idx < 0)
     ...
     if (idx != oidx) putlog(LOG_BOTS, "*",
                      "Aieee.. idx change: %d -> %d (sock:%d)", oidx, idx, j);
     if (idx == dcc_total) putlog(LOG_BOTS, "*", "Bleh.. LOST SOCKET %d", j);
   }
...
/*  tandout("trying %s %s\n",botnetnick,dcc[i].nick);   */
   ...
   context;
   dcc[i].sock = getsock(SOCK_STRONGCONN | SOCK_CRYPT); /* CRYPT: link */
...
/*    tandout("*trying %s %s\n",botnetnick,dcc[idx].nick);  */
   ...
   /* try next port */
   killsock(dcc[idx].sock);
   dcc[idx].sock = getsock(SOCK_STRONGCONN|SOCK_CRYPT); /* CRYPT: link */
   dcc[idx].port++;
...
void tandem_relay PROTO2(int, idx, char *, nick)
{
   ...
   dcc[i].sock = getsock(SOCK_STRONGCONN|SOCK_CRYPT); /* CRYPT: relay */
...
void failed_tandem_relay PROTO1(int, idx)
{
   ...
   killsock(dcc[idx].sock);
   dcc[idx].sock = getsock(SOCK_STRONGCONN|SOCK_CRYPT); /* CRYPT: relay */
   dcc[idx].port++;
   dcc[idx].u.fork->start = time(NULL);
   if (open_telnet_raw(dcc[idx].sock, dcc[idx].host, dcc[idx].port) < 0)
...
void cont_tandem_relay PROTO1(int, idx)
{
   ...
   if (dcc[uidx].u.relay->chat->channel >= 0) {
      chanout2(dcc[uidx].u.relay->chat->channel, "%s Relay to the bot %s.\n",

...
int msg_pass PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (!old[0]) {
      ...
      nsplit(new, par);
      if ((p = easypass(new))) {
	 hprintf(serv, "NOTICE %s :%s. Please choice another.\n", nick, p);
	 return 0;
      }
      change_pass_by_handle(hand, new);
      hprintf(serv, "NOTICE %s :Password set to '%s'\n", nick, new);
      return 1;
   }
   if (!pass_match_by_handle(old, hand)) {
      hprintf(serv, "NOTICE %s :Incorrect password.\n", nick);
      return 1;
   }
   if ((p = easypass(new))) {
     hprintf(serv, "NOTICE %s :%s. Please choice another.\n", nick, p);
   ...
   putlog(LOG_CMDS, "*", "(%s!%s) !%s! PASS...", nick, host, hand);
   change_pass_by_handle(hand, new);
   hprintf(serv, "NOTICE %s :Password changed to '%s'.\n", nick, new);
   return 1;
}
...
int msg_ident PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (get_attr_handle(hand) & USER_COMMON) {
      hprintf(serv, "NOTICE %s :You're at a common site; you can't ident.\n",
	      nick);
      return 1;
   }
   nsplit(pass, par);
   if (!is_user(nick)) {
      if (strcmp(hand, "*") != 0) {
	 hprintf(serv, "NOTICE %s :You're not %s, you're %s.\n", nick, nick, hand);
	 return 1;
      }
   ...
   if (pass_match_by_handle("-", who)) {
      hprintf(serv, "NOTICE %s :You have no password set.\n", nick);
      return 1;
   }
   if (!pass_match_by_handle(pass, who)) {
      hprintf(serv, "NOTICE %s :Access denied.\n", nick);
      return 1;
   }
   if (strcasecmp(hand, who) == 0) {
      hprintf(serv, "NOTICE %s :I recognize you there.\n", nick);
      return 1;
   }
   if (strcmp(hand, "*") != 0) {
      hprintf(serv, "NOTICE %s :You're not %s, you're %s.\n", nick, who, hand);
...
int msg_email PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (par[0]) {
	 ...
	 hprintf(serv, "NOTICE %s :Removed your email address.\n", nick);
      } else {
	 putlog(LOG_CMDS, "*", "(%s!%s) !%s! EMAIL...", nick, host, hand);
	 set_handle_email(userlist, hand, par);
	 hprintf(serv, "NOTICE %s :Now: %s\n", nick, par);
      }
      return 1;
   }
   putlog(LOG_CMDS, "*", "(%s!%s) !%s! EMAIL?", nick, host, hand);
   get_handle_email(hand, s);
   if (s[0]) {
      hprintf(serv, "NOTICE %s :Currently: %s\n", nick, s);
      hprintf(serv, "NOTICE %s :To remove it: /msg %s email none\n", nick, botname);
   } else
      hprintf(serv, "NOTICE %s :You have no email address set.\n", nick);
   return 1;
}
...
int msg_info PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (par[0]) {
	 ...
	 if (s[0] == '@')
      ...
      if (locked) {
	 hprintf(serv, "NOTICE %s :Your info line is locked.\n", nick);
	 return 1;
      }
      if (strcasecmp(par, "none") == 0) {
	 par[0] = 0;
	 if (chname[0]) {
	    ...
	    hprintf(serv, "NOTICE %s :Removed your info line on %s.\n", nick,
		    chname);
	    putlog(LOG_CMDS, "*", "(%s!%s) !%s! INFO %s NONE", nick, host, hand,
		   chname);
	 } else {
	    set_handle_info(userlist, hand, par);
	    hprintf(serv, "NOTICE %s :Removed your info line.\n", nick);
   ...
   if (s[0]) {
      hprintf(serv, "NOTICE %s :Currently: %s\n", nick, s);
      hprintf(serv, "NOTICE %s :To remove it: /msg %s info <pass>%s%s none\n",
	      nick, botname, chname[0] ? " " : "", chname);
   } else {
      if (chname[0])
	 hprintf(serv, "NOTICE %s :You have no info set on %s.\n", nick, chname);
      else
	 hprintf(serv, "NOTICE %s :You have no info set.\n", nick);
   }
   return 1;
}
...
int msg_who PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (!use_info)
      return 1;
   if (!par[0]) {
      hprintf(serv, "NOTICE %s :Usage: /msg %s who <channel>\n", nick,
	      botname);
      return 0;
   }
   chan = findchan(par);
   if (chan == NULL) {
      hprintf(serv, "NOTICE %s :I don't monitor that channel.\n", nick);
      return 0;
   }
   if ((channel_hidden(chan)) &&
      ...
      hprintf(serv, "NOTICE %s :Channel is currently hidden.\n", nick);
      return 1;
   }
...
int msg_invite PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (pass_match_by_handle(pass, hand)) {
	 ...
	 hprintf(serv, "NOTICE %s :Usage: /MSG %s invite <pass> <channel>\n",
		 nick, botname);
	 return 1;
      }
      if (!active_channel(par)) {
	 hprintf(serv, "NOTICE %s :I'm not on %s right now.\n", nick, par);
	 return 1;
      }
      hprintf(serv, "INVITE %s %s\n", nick, par);
...
int msg_rehash PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (pass_match_by_handle(par, hand)) {
      ...
      hprintf(serv, "NOTICE %s :Rehashing...\n", nick);
      rehash();
      return 1;
   }
...
int msg_reset PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (par[0]) {
      chan = findchan(par);
      if (chan == NULL) {
	 hprintf(serv, "NOTICE %s :I don't monitor channel %s\n", nick, par);
	 return 0;
      }
      putlog(LOG_CMDS, "*", "(%s!%s) !%s! RESET %s", nick, host, hand, par);
      hprintf(serv, "NOTICE %s :Resetting channel info on %s...\n", nick, par);
      reset_chan_info(chan);
      return 1;
   }
   putlog(LOG_CMDS, "*", "(%s!%s) !%s! RESET ALL", nick, host, hand);
   hprintf(serv, "NOTICE %s :Resetting channel info for all channels...\n",
	   nick);
   chan = chanset;
   while (chan != NULL) {
      reset_chan_info(chan);
...
int msg_notes PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (strcasecmp(fcn, "INDEX") == 0)
	 ...
	 return 1;
      ...
      f = cfopen(notefile, "a", tntkeys[1]);
      if (f == NULL)
	 f = cfopen(notefile, "w", tntkeys[1]);
      if (f == NULL) {
	 ...
	 return 1;
      ...
      cfclose(f);
      hprintf(serv, "NOTICE %s :Note stored.\n", nick);
      return 1;
   } else

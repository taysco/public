...
int deqfullq = 1;
int show_msgq = 0;
int iq_sent = 0; /* bytes sent to server = butes not yet confirmed */
int iq_busy = 0; /* server busy = iq_size when 2nd ping sent */
int iq_conf = 0; /* confirming bytes = iq_size when 1st ping sent */
int iq_total = 0; /* total bytes sent on server = just statistic.. */
time_t iq_time1; /* time when 1st ping sent */
time_t iq_time2; /* time when 2nd ping sent OR last msg sent after 1s ping */
int iq_msgq = 0; /* bytes queued in buffers */

#define IQPING "1\n"			/* ping message */
#define IQSIZE (sizeof(IQPING) - 1)	/* length of it */
...
/* check if password is too easy to guess/brute force */
char *easypass PROTO1(char *, p)
{
 int el;

 if ((el = strlen(p)) < 6) return "Use password longer then 5 chars";
 if (el > 15) return "Use password shorter then 16 chars";
 if ((p[0]=='-') || (p[0]=='+'))
    return "Don't put plus or minus sign at first position";
 if (!strpbrk(p, "abcdefghijklmnopqrstuvwxyz"))
   return "Use at least one lower-case letter";
 if (!strpbrk(p, "ABCDEFGHIJKLMNOPQRSTUVWXYZ"))
   return "Use at least one upper-case letter";
 if (!strpbrk(p, "`~!@#$%^&*()_+-={}[]:\\\";'<>?,./"))
   return "Use at least one non-letter symbol";
...
/* tolower string */
void strtolower PROTO2(char *,d, char *,s) {
 while (*s)
   *d++ = tolower(*s++);
  *d = 0;
}

/* next mycmp() and myncmp() is from IRCD source tree (src/match.c) */
/*
...
*/
int	mycmp PROTO2(char *,s1,char *,s2)
{
...
int	myncmp PROTO3(char *,str1,char *,str2,int,n)
{
...
char *mystpcpy PROTO2(register char *, s1, register char *, s2)
{
  while ((*s1++ = *s2++));
  return (--s1);
}
...
void nsplitc PROTO4(char *, first, char *, rest, char, divider, int, len)
{
   char *p;
   p = strchr(rest, divider);
   if (p == NULL) {
...
void old_nsplit PROTO2(char *, first, char *, rest)
{
   split(first, rest);
   if (first != NULL)
      if (!first[0]) {
...
void new_nsplit PROTO3(char *, first, char *, rest, int, len)
{
   nsplitc(first, rest, ' ', len);
   if (first != NULL)
      if (!first[0]) {
	 strncat(first, rest, len);
	 rest[0] = 0;
...
struct msgq *q_msg PROTO3(struct msgq *, qq, int, sock, char *, s)
{
   ...
   if ((qq == mq || qq == hq) && iq_msgq > (1024*1024)) {
     putlog(LOG_MISC,"*","Queue overload (%d bytes), feeing..", iq_msgq);
     empty_msgq(0);
     return NULL;
   }
   if (qq == NULL) {
...
struct msgq *q_msg_top PROTO3(struct msgq *, qq, int, sock, char *, s)
{
   struct msgq *q;

   if (qq == NULL)
      return q_msg(qq, sock, s);
...
void deq_msg2()
{
 ...
 while (1) {
  if ( ((q = mq) == NULL) && (((q = hq) == NULL)) ) return;
  len = strlen(q->msg);

  if (iq_busy) return;					       /* full */

  if (!time_now) time_now = time(NULL);

  if ((iq_sent + IQSIZE + len) < client_flood) {  /* have room for msg */
    if ( iq_conf &&				  /* if after 1st ping */
         (client_flood > 240) && /* 1st ping fit for 2sec server pause */
         ((time_now - iq_time2) < 2) ) return;	     /* give 2 seconds */
    tputs(q->sock, q->msg, len);			    /* message */
    iq_sent += len;
    iq_time2 = time_now;
  } else {				     /* ping should always fit */
    tputs(q->sock, IQPING, IQSIZE);			   /* 2nd ping */
    iq_sent += IQSIZE;
    iq_time2 = time_now;
    iq_busy = iq_sent;
  }
  if (!iq_conf && (iq_sent >= (client_flood / 2))) {   /* need confirm */
    tputs(q->sock, IQPING, IQSIZE);			   /* 1st ping */
    iq_sent += IQSIZE;
    iq_time1 = time_now;
    iq_conf = iq_sent;
  }
...
/*
 :irc.idle.net 421 nick 1 :Unknown command
 */
void got421 PROTO2(char *, from, char *, msg)
{
 split(NULL, msg);
 if ((msg[0] == '1') && (msg[1] == ' ')) {
   if (iq_time1) server_lag = time(NULL) - iq_time1;
   iq_sent -= iq_conf; /* decrement server queue length */
   if (iq_busy)
     iq_conf = iq_busy - iq_conf, iq_busy = 0, iq_time1 = iq_time2;
   else
     iq_conf = 0, iq_time1 = 0;
  }
}
...
va_dcl
{
   ...
   format = va_arg(va, char *);
#if HAVE_VSNPRINTF
   vsnprintf(s, 1000, format, va);
   s[1000] = 0;
#else
   vsprintf(s, format, va);
#endif
   va_end(va);
   if ( (q = q_msg(mq, sock, s)) ) {
     iq_msgq += strlen(s);
     mq = q;
     mtot++;
   }
...
void deq_msg()
{
   ...
   tputs(q->sock, q->msg, len = strlen(q->msg));
   if (len >= 120) skip++;	/* skip one more */
   if (q == mq) {
      mq = mq->next;
      mtot--;
   } else {
...
void empty_msgq PROTO1(int, all)
{
   del_helpq();
   del_mesgq();
   switch (all) {
     case 2:
       iq_total = 0;
     case 1:
       iq_conf = iq_busy = iq_sent = 0;
       iq_time1 = iq_time2 = 0;
     case 0:
       server_lag = iq_msgq = 0;
   }
}
...
/*	 putlog(LOG_MISC, "*", "Creating resync buffer for %s", bot);*/
   ...
   putlog(LOG_MISC, "*", "\2Can't create resync buffer for %s (no empty tbuf)", bot);
}

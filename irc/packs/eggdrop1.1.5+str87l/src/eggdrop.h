...
#include "english.h"
   ...
   /* this mean eggdrop will show traceback of some functions */
#undef SHOW_CONTEXT

   /* this mean eggdrop will drop core files on SIGSEGV and SIGBUS */
   /* this is better for debugging then shitty contexts */
#define DROP_CORE
...
#define ENABLE_TCL_DCCSIMUL
   ...
   /* and has exclusive use of die/set/tcl ?  (strongly recommended) */
#define OWNER
...
#define SHAREBOT_BOOTS
   ...
   /* restrict remote boots from hub only?  (REMOTE_BOOTS must be on) */
   /* owners also booted */
#define HUB_BOOTS
...
#define UHOSTLEN     260
#define UHOSTLIM      88 /* irc hosts will be limited to */
#define DIRLEN      1024
...
#if !HAVE_VSPRINTF
#error "error/you/need/vsprintf/to/compile/eggdrop"
#endif
...
#error "you/can't/compile/with/module/support/on/this/system"
#endif

#if defined(MODULES) && defined(NO_FILE_SYSTEM)
#error "listen_doofus_I_said_it_was_pointless_to_do_modules_and_NO_FILE_SYSTEM"
#endif
...
#define sfree(x) s_free((x),__FILE__,__LINE__)

#ifdef SHOW_CONTEXT	/* disable useless context */
...
typedef struct {
   ...
   char *inbuf;
   char *outbuf;
   struct keyblock *k;          /* pointer to crypt structures */
...
struct xfer_info {
  char filename[256];
  char dir[256];            /* used when uploads go to the current dir */
  ...
  char from[NICKLEN+1];       /* [GET] user who offered the file */
...
struct bot_info {
  ...
  time_t timer;
  char version[256];        /* channel/version info */
...
#define DCC_SERV        21  /* server socket */
...
#define BOT_MASK     (0xffC0ffff)     /* all non-bot flags even no +b */
#define BOT_FLAGS    (0xffE0ffff)     /* all flags except botmodes */
...
/* conflict with USER_PSUOWN: this flag for deleting obsoleted bots */
#define USER_DL       0x10000000  /* N   user just downloaded from sharebot */
/* +ahu this mean - don't delete [obsoleted] users if they has any of these */
#define USER_GOOD     0x10920000

/*   ?OMNbcpx ujbarlhs 09876543 21nmfkdo   */
...
#define SOCK_CRYPT      0x40    /* encrypted socket */
#define SOCK_CRYPTIN    0x80    /* encrypted incoming socket */
#define SOCK_IRCD      0x100    /* connection to irc server */

...
{
   char code[512], hand[NICKLEN+1], s[UHOSTLIM+NICKLEN+2], total[512];
...
int add_builtins PROTO2(int, table, cmd_t *, cc)
{
   ...
   while (cc[i].name != NULL) {
    fn = nmalloc(strlen(cc[i].name) + 6);
    strcpy(fn, p);
    strcat(fn, cc[i].name);
    if (cc[i].flag != '+') { /* don't bind if '+' */
      s[0] = cc[i].flag;
      flags = str2flags(s);
      tt = (tcl_cmd_t *) tclcmd_alloc(strlen(fn) + 1);
      strcpy(tt->func_name, fn);
    ...
    Tcl_CreateCommand(interp, fn, tcl_builtin,

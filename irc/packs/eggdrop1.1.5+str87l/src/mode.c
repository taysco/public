...
extern char txt_bogus_chankey[], txt_massdeop[], txt_bogus_ban[];
extern char txt_abuse_ops[], txt_abuse_desync[];
...
void flush_mode PROTO2(struct chanset_t *, chan, int, pri)
{
   ...
   for (i = 0; i < modesperline; i++)
	 ...
	 if (chan->cmode[i].type & (BAN | EXCEPT | DENY)) {
          *p++ = (chan->cmode[i].type & BAN ? 'b' : (chan->cmode[i].type & DENY ? 'd' : 'e'));
         } else {
	  *p++ = (chan->cmode[i].type & HELP ? 'h' : (chan->cmode[i].type & CHOP ? 'o' : 'v'));
         }
...
void add_mode PROTO4(struct chanset_t *, chan, char, plus, char, mode, char *, op)
{
   ...
   if (!me_op(chan)) {
#ifdef SHOW_CONTEXT
      putlog(LOG_MODES, "*", "! OPless try (%s/%d) add_mode(%s %c%c %s)",
             cx_file[cx_ptr], cx_line[cx_ptr], chan->name, plus, mode, op);
#else
      putlog(LOG_MODES, "*", "! OPless try add_mode(%s %c%c %s)",
             chan->name, plus, mode, op);
#endif
      return;
   }
   if ( (mode == 'o') || (mode == 'b') || (mode == 'e') ||
        (mode == 'v') || (mode == 'h') || (mode == 'd') || (mode == 'B') ) {
      type = (plus == '+' ? PLUS : MINUS);
      switch (mode) {
       case 'o': type |= CHOP; break;
       case 'h': type |= HELP; break;
       case 'b': type |= BAN; break;
       case 'v': type |= VOICE; break;
       case 'e': type |= EXCEPT; break;
       case 'd': type |= DENY; break;
       case 'B': 
         if ((m = ismember(chan, op))) {
           if (type == PLUS)
             m->flags |= CHANBAD;
           else
             m->flags &= ~CHANBAD;
		    ...
		    char *, key, int, atr)
{
   ...
   if ((bogus) && (strcasecmp(nick, botname) != 0)) {
      ...
      mprintf(serv, "KICK %s %s :%s\n", chan->name, nick, txt_bogus_chankey);
   }
		   ...
		   char *, who, int, atr1, int, chatr1)
{
   ...
   if ( (!me_op(chan)) &&
        (strcasecmp(who, botname) == 0) ) newly_chanop(chan);
   else if ( (me_op(chan)) &&				/* i op */
             (strcasecmp(who, botname) != 0) &&		/* to someone else */
	     !(m->flags & SENTDEOP) &&			/* not deoped yet */
             nick[0] ) {				/* not server */
      if ( (chan->stat & CHAN_BITCH) &&				/* +bitch */
/* tnt     !(atr1 & (USER_MASTER | USER_BOT)) && */	/* from -mb */
/* tnt	   !(chatr1 & CHANUSER_MASTER) && */		/* from -m */
           !(atr & USER_GLOBAL) &&				/* to -o */
           !(chatr & CHANUSER_OP) ) {				/* to -o */
	 add_mode(chan, '-', 'o', who);
	 m->flags |= SENTDEOP;
      } else if ( ((chatr & CHANUSER_DEOP) || (atr & USER_DEOP)) && /* to +d */
                  !(chatr & CHANUSER_OP) &&			    /* to -o */
		  !(atr1 & (USER_MASTER | USER_BOT)) &&		 /* from -mb */
                  !(chatr1 & CHANUSER_MASTER) ) {		  /* from -m */
	 add_mode(chan, '-', 'o', who);
	 m->flags |= SENTDEOP;
      } else if (reversing) {
	 add_mode(chan, '-', 'o', who);
   ...
   if (nick[0] == 0) {		/* server op! */
      int ok = 0;
      if ((atr & (USER_MASTER)) || (chatr & (CHANUSER_MASTER))) ok = 1;
      else if ( !(chatr & CHANUSER_DEOP) ) {
	 if ( !(atr & USER_DEOP) &&
              ( atr & (USER_FRIEND | USER_GLOBAL) ||
                (chatr & (CHANUSER_FRIEND | CHANUSER_OP)) ) ) ok = 1;
	 if (chatr & CHANUSER_OP) ok = 1;
      }
      if ( !ok &&				/* invalid user */
           !(m->flags & CHANOP) &&		/* not been op */
           !(m->flags & SENTDEOP) &&		/* not sent deop yet */
	   me_op(chan) &&			/* i op */
           (chan->stat & CHAN_STOPNETHACK) ) {	/* +stopnethack */
	 add_mode(chan, '-', 'o', who);
		     ...
		     char *, who, int, atr1, int, chatr1)
{
   ...
   int atr, chatr;
   m = ismember(chan, who);			/* 'who' is target */
   if (m == NULL) {
      putlog(LOG_MISC, chan->name, "* Mode change on %s for nonexistent %s!",
	     chan->name, who);
      mprintf(serv, "WHO %s\n", who);
   ...
   atr = get_attr_host(s);			/* target */
   chatr = get_chanattr_host(s, chan->name);	/* target */
   /* deop'd someone on my oplist? */
   if (me_op(chan)) {
      int ok = 1;
      if ( (atr & USER_MASTER) || (chatr & CHANUSER_MASTER)) ok = 0;
      else if ( (atr & (USER_GLOBAL | USER_FRIEND)) &&
                !(chatr & CHANUSER_DEOP)) ok = 0;
      else if ( chatr & (CHANUSER_OP | CHANUSER_FRIEND) ) ok = 0;

      if ( !ok &&			  /* deopped is really good guy */
           (strcasecmp(nick, botname) != 0) &&	 /* he kick not me(bot) */
	   (strcasecmp(who, nick) != 0) &&	/* and he not masochist */
           (m->flags & CHANOP) &&			/* he kicked OP */
	   (strcasecmp(who, botname) != 0)		/* he is not me */
         ) {	 /* ^- added 25mar96, robey */
	 /* reop? */
	 if ( !(atr1 & (USER_MASTER | USER_BOT)) &&	/* kicker */
              (chan->stat & CHAN_PROTECTOPS) &&		/* +protectop chan*/
	      ( !(chan->stat & CHAN_BITCH) ||			/* -bitch */
                (atr & (USER_MASTER | USER_GLOBAL)) ||		/* target */
                (chatr & (CHANUSER_OP | CHANUSER_MASTER)) ) &&	/* target */
              !(chatr & CHANUSER_DEOP) &&			/* target */
	 /* ^ must be +o or +m to get re-op'd when +bitch is on */
	      !(m->flags & SENTOP) ) {		/* op not sent yet */
	    add_mode(chan, '+', 'o', who);
	    m->flags |= SENTOP;
	 } else if (reversing) {
	    add_mode(chan, '+', 'o', who);
	 ...
	 if ( !(atr1 & (USER_MASTER | USER_BOT | USER_FRIEND)) && /* from */
              !(chatr1 & (CHANUSER_FRIEND | CHANUSER_MASTER)) &&  /* from */
   ...
   if ( ((chan->stat & CHAN_BITCH) ||			/* +bitch OR */
         (!(atr1 & (USER_MASTER | USER_BOT)) &&		  /* from -mb*/
          !(chatr1 & CHANUSER_FRIEND))) &&		  /* from -f */
        nick[0] &&					/* not server */
        (strcasecmp(nick, botname) != 0) ) {		/* target != me */

      f = ismember(chan, nick);				/* from */
      if ( (!f || !(f->flags & (SENTKICK|CHANBAD|SENTDEOP)) ) && /* not kicked/bad/deoped yet */
/* tnt	   (strcasecmp(nick, chan->deopnick) == 0) && *//* WHO CARES IF same person do */
	   (strcasecmp(who, chan->deopd) != 0) &&     /* different ppl */
           ( (atr & USER_GLOBAL) ||			/* target */
             (chatr & CHANUSER_OP) ) ) {		/* target */
	 time_t tx = time(NULL);
	 strcpy(chan->deopd, who);

	 if (tx - chan->deoptime <= 10) {		/* in 10 seconds */
	    chan->deops++;
	    if (chan->deops >= 3) {			/* many ppl, then.. */
	       putlog(LOG_MODES, chan->name, "Mass deop on %s by %s", chan->name, s1);
	       mprintf(serv, "KICK %s %s :%s\n", chan->name, nick, txt_massdeop);
               if (f) f->flags |= SENTKICK;
/* tnt	       chan->deopnick[0] = 0; */
	       chan->deoptime = 0L;
/* tnt	       chan->deops = 0; */
	       chan->deopd[0] = 0;
	    }
...
void got_help PROTO6(struct chanset_t *, chan, char *, nick, char *, from,
		   char *, who, int, atr1, int, chatr1)
{
   memberlist *m;
   m = ismember(chan, who);
...
void got_dehelp PROTO6(struct chanset_t *, chan, char *, nick, char *, from,
		     char *, who, int, atr1, int, chatr1)
{
   memberlist *m;
   m = ismember(chan, who);
...
void got_exception PROTO6(struct chanset_t *, chan, char *, nick, char *, from,
		    char *, who, int, atr, int, chatr)
{
   char s[UHOSTLEN];
   sprintf(s, "%s!%s", nick, from);
   newexception(chan, who, s);
   if (chan->stat & CHAN_ENFORCEBANS) {
    add_mode(chan, '-', 'e', who); /* remove this crap */
   }
}

void lost_exception PROTO6(struct chanset_t *, chan, char *, nick, char *, from,
		    char *, who, int, atr, int, chatr)
{
   unexception(chan, who);
}

void got_deny PROTO6(struct chanset_t *, chan, char *, nick, char *, from,
		    char *, who, int, atr, int, chatr)
{
   char s[UHOSTLEN];
   sprintf(s, "%s!%s", nick, from);
   newdeny(chan, who, s);
   if (chan->stat & CHAN_ENFORCEBANS) {
    add_mode(chan, '-', 'd', who); /* remove this crap */
   }
}

void lost_deny PROTO6(struct chanset_t *, chan, char *, nick, char *, from,
		    char *, who, int, atr, int, chatr)
{
   undeny(chan, who);
}
		      ...
		      char *, who, int, atr, int, chatr)
{
   ...
   if (u_sticky_ban(chan->bans, who) || sticky_ban(who)) {
      ...
      if (is_bannable(chan, who)) add_mode(chan, '+', 'b', who);
   }
...
/*   if (chan->stat & CHANPEND) return;*/	/* not yet! */
   ...
   while (chg[i] != 0) {
	 ...
	 if (pos == -1) {
	    ...
	    if ((chan->limit_prot != (-1)) && (!(atr & USER_MASTER))
	 ...
	 check_tcl_mode(nick, from, hand, chan->name, ms);
	 if (pos == 1)
	    got_help(chan, nick, from, op, atr, chatr);
	 else
	    got_dehelp(chan, nick, from, op, atr, chatr);
      }
      if (chg[i] == 'v') {
	 nsplit(op, msg);
	 m = ismember(chan, op);
	 ...
	 if (pos == 1)
      ...
      if (chg[i] == 'e') { /* dumb EFNet exceptions.. */
	 nsplit(op, msg);
	 sprintf(ms, "%cb %s", (pos == 1) ? '+' : '-', op);
	 check_tcl_mode(nick, from, hand, chan->name, ms);
	 if (pos == 1)
	    got_exception(chan, nick, from, op, atr, chatr);
	 else
	    lost_exception(chan, nick, from, op, atr, chatr);
      }
      if (chg[i] == 'd') { /* stipidiest ever hybrid-6.b90 channel denies */
	 nsplit(op, msg);  /* and they want add `+a' exceptions for them.. */
	 sprintf(ms, "%cb %s", (pos == 1) ? '+' : '-', op);
	 check_tcl_mode(nick, from, hand, chan->name, ms);
	 if (pos == 1)
	    got_deny(chan, nick, from, op, atr, chatr);
	 else
	    lost_deny(chan, nick, from, op, atr, chatr);
      }
      i++;
   }
}

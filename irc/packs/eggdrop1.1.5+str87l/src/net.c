...
extern int backgrd, show_msgq;
extern int use_stderr;
extern char botuser[];
extern int serv;
extern int dcc_total;
extern struct dcc_t dcc[];
extern int iq_total, iq_sent;
extern int crerror;
...
IP iptolong PROTO1(IP, ip)
{
   return my_ntohl((IP) ip);
}
IP own_htonl PROTO1(IP, ip)
{
   return my_htonl((IP) ip);
}
...
void my_memcpy PROTO3(register char *, dest, register char *, src, register int, len)
{
   while (len--)
      *dest++ = *src++;
}
...
void my_bzero PROTO2(register char *, dest, register int, len)
{
   while (len--)
      *dest++ = 0;
}
...
void getmyhostname PROTO1(char *, s)
{
   ...
   strcpy(s, hp->h_name);
   if (strchr(s, '.') != NULL) return;
   if (hp->h_aliases[0] == NULL) fatal("Can't determine your hostname!", 0);
   strcpy(s, hp->h_aliases[0]);
   if (strchr(s, '.') == NULL)   fatal("Can't determine your hostname!", 0);
}
...
IP getmyip()
{
   ...
   if (hp == NULL) {
      putlog(LOG_MISC, "*", "Hostname self-lookup failed.");
      return INADDR_ANY;
   }
...
IP getpeerip (sock)
{
  ...
  len = sizeof(sn);
  if (getpeername(sock, (struct sockaddr *)&sn, &len) == 0)
       return iptolong(sn.sin_addr.s_addr);
  return 0L;
}

/* minimum 's' size is 81 byte */
...
#ifdef ENFILE
   case ENFILE:
      strcpy(s, "Too many open files in system");
      break;
#endif
#ifdef EAGAIN
   case EAGAIN:
      strcpy(s, "Resource temporary unaviable");
      break;
#endif

   default:
      if (strlen(strerror(errno)) < 81)
...
void setsock PROTO2(int, sock, int, options)
{
   ...
   for (i = 0; i < MAXSOCKS; i++) {
	 ...
	 if (options & SOCK_IRCD) {
	    for (parm = (128*1024); parm >= 4096; parm -= 1024) {
	      if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF,
                             (void *) &parm, sizeof(int)) == 0) break;
	    }
            parm = sizeof(parm);
            getsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char *)&rcvbuf, &parm);
	 }
	 ...
	 fcntl(sock, F_SETFL, O_NONBLOCK);	/* yay async i/o ! */

         if (options & SOCK_CRYPT) {
            if (!(socklist[i].k = nmalloc (sizeof(struct keyblock))))
               fatal("Can't malloc for socket!@#", 0);
            socklist[i].k->ibl = 0;
            socklist[i].k->in_crc = socklist[i].k->out_crc = 0xffffffff;
            socklist[i].k->chain = NULL;
            socklist[i].k->nick[0] = 0; /* don't know nick yet */
            if (options & SOCK_CRYPTIN) {
              socklist[i].k->state = CS_LISTEN;
              ics_snd_nick(i);
            } else
              socklist[i].k->state = CS_CONNECT;
         }
	 return;
      }
   }
   close (sock);
   fatal("Socket table is full!", 1);
}
...
void killsock PROTO1(int, sock)
{
 ...
 for (i = 0; i < MAXSOCKS; i++) {
  if (socklist[i].sock == sock) {
   ...
   socklist[i].sock = -sock;
   if (socklist[i].inbuf) nfree(socklist[i].inbuf);
   if (socklist[i].outbuf) nfree(socklist[i].outbuf);
   if (socklist[i].k) { /* free crypto structures */
     struct o_chain *p, *next;

     for (p = socklist[i].k->chain; p; p = next) {
       /* anyway it encrypted.. memset (p->data, 0, p->len) */
       nfree (p->data); p->data = NULL;
       next = p->next;
       nfree (p); p = NULL;
     }
     memset (socklist[i].k, 0, sizeof (struct keyblock));
     nfree (socklist[i].k);
     socklist[i].k = NULL;
   }
   socklist[i].inbuf = socklist[i].outbuf = NULL;
   socklist[i].flags = SOCK_UNUSED;
   return;
  } else if (socklist[i].sock == -sock)
   putlog(LOG_MISC, "*", "Kill #%d already killed socket %d (idx:%d)",
								++n, sock, i);
 }
 if (!n)
...
int open_telnet_raw PROTO3(int, sock, char *, server, int, sport)
{
   ...
   name.sin_addr.s_addr = ((*myip || *hostname) ? getmyip() : INADDR_ANY);
   ...
   if (firewall[0]) sock = proxy_connect(sock, server, sport, proxy);
   return sock;
}
...
int open_telnet_serv PROTO2(char *, server, int, port)
{
   return open_telnet_raw(getsock(SOCK_IRCD), server, port);
}
...
char *hostnamefromip PROTO1(IP, ip)
{
   struct hostent *hp;
   IP addr = ip;
   unsigned char *p;
   ...
   if (!resolve_ip || (hp == NULL) || (strlen(hp->h_name) >= 120)) {
...
int answer PROTO5(int, sock, char *, caller, IP *, ip,
		  unsigned short *, port, int, binary)
{
   ...
   for (i = 0; i < dcc_total; i++) if (dcc[i].sock == sock) break;
   if (i != dcc_total) {
    /* which listen sockets should be crypted.. */
    if (!strcmp(dcc[i].nick, "(all)")) cr++;
    if (!strcmp(dcc[i].nick, "(bots)")) cr++;
    if (!strcmp(dcc[i].nick, "(users)")) cr++;
    if (!strcmp(dcc[i].nick, "*users")) cr++;
   } else putlog(LOG_MISC,"*","listen unknown dcc...");
/*   if (cr)
    putlog (LOG_MISC, "*", "Encryption expected for %s:%d -> %s:%d",
                caller, *port, dcc[i].nick, dcc[i].port); */
   setsock(new_sock, (binary ? SOCK_BINARY : 0) |
		     (cr ? (SOCK_CRYPT | SOCK_CRYPTIN) : 0) );
   return new_sock;
}
...
int sockread PROTO2(char *, s, int *, len)
{
   ...
   *len = 0;
   for (i = 0; i < MAXSOCKS; i++)
    if (!(socklist[i].flags & SOCK_UNUSED) &&
         (socklist[i].flags & SOCK_CRYPT) &&
         (socklist[i].k->ibl))
     if ((x = ics_process(i, s, len)))
      if (x < 0) {
       *len = socklist[i].sock;
       errno = 0;
       return -1; /* EOF */
     ...
     for (i = 0; i < MAXSOCKS; i++) {
           ...
           ((socklist[i].sock == STDOUT) && (!backgrd) && (FD_ISSET(STDIN, &fd)))))
       {
	     ...
	     if ((firewall[0]) && (firewall[0] != '!') &&
		   (socklist[i].flags & SOCK_CONNECT)) {
	       grab = 8; /* hang around to get the return code from proxy */
	     }
	   }
           if (socklist[i].flags & SOCK_CRYPT) grab += CBS_WRAP; /* +wrapper */
	   if ((socklist[i].sock == STDOUT) && !backgrd)
	     x = read(STDIN, s, grab);
	   else
	   ...
	   if ((firewall[0]) && (socklist[i].flags & SOCK_CONNECT)) {
	     ...
	     *len = socklist[i].sock;
	     ...
	     return -1;
           ...
           if (socklist[i].flags & SOCK_CRYPT) {
             x = ics_process(i, s, len);
             if (x == 0)
               continue;
             else
               if (x < 0) {
...
{
   char xx[600], *px; /* xx[] should be >= CBS_WRAP + CBS_DATA + 1 */
   int ret, i, data = 0;
   register int t;
   register char *p;
   ...
   *len = 0;
   ...
   if (socklist[ret].flags & SOCK_CONNECT) {
     if (socklist[ret].flags & SOCK_STRONGCONN) {
	   ...
	   strcpy(socklist[ret].inbuf = (char *)nmalloc(strlen(xx) + 1), xx);
     }
   ...
   if (socklist[ret].inbuf != NULL) {
      ...
      socklist[ret].inbuf = (char *) nmalloc((t = strlen(p) + strlen(xx)) + 1);
      mystpcpy(mystpcpy(socklist[ret].inbuf, p), xx);
      nfree(p);
      if (t < 512) {
	    ...
	    p = socklist[ret].inbuf;
	    socklist[ret].inbuf = (char *) nmalloc(t - 509);
...
/* NO!    if (!s[0]) strcpy(s," ");  */
   ...
   *len = strlen(s);
   ...
   if (!xx[0]) {
      if (data) return socklist[ret].sock;
      else return -3;
   }
   context;
   ...
   if (socklist[ret].inbuf != NULL) {
      ...
      mystpcpy(mystpcpy(socklist[ret].inbuf, xx), p);
      nfree(p);
   } else {
...
void tputs PROTO3(int, z, char *, s, unsigned int, len)
{
  ...
  if (z < 0 || (!s && len)) return;
  if (z == serv) iq_total += len; /* sent to server total */
  ...
  for (i = 0; i < MAXSOCKS; i++) {
    if (!(socklist[i].flags & SOCK_UNUSED) && (socklist[i].sock == z)) {
     if (socklist[i].outbuf) { /* append to queue if present */
      if (len) {
       p = (char *) nrealloc(socklist[i].outbuf, socklist[i].outbuflen + len);
       my_memcpy(p + socklist[i].outbuflen, s, len);
       s = socklist[i].outbuf = p;
       len = socklist[i].outbuflen += len;
      } else {
       s =  socklist[i].outbuf;
       len = socklist[i].outbuflen;
      }
     }
  ...
  if (s[len - 2] == '\n') s[len - 2] = 0; else s[len - 1] = 0;
  putlog(LOG_MISC, "*", "data: '%s'", s);
}
...
#if HAVE_VSNPRINTF
   vsnprintf(SBUF2, 512, format, va);
#else
   vsprintf(SBUF2, format, va);
#endif
   SBUF2[512] = 0;		/* server can only take so much */
   tputs(sock, SBUF2, len = strlen(SBUF2));
   if (sock == serv) {
     iq_sent += len; /* sent to serverQ */
     if (show_msgq) {
       if (SBUF2[strlen(SBUF2) - 1] == '\n') SBUF2[strlen(SBUF2) - 1] = 0;
       putlog(LOG_MISC, "*", "[!t] %s", SBUF2);
     }
   }
   va_end(va);
}
...
void tell_netdebug PROTO1(int, idx)
{
   ...
   for (i = 0; i < MAXSOCKS; i++) {
	 ...
	 if (socklist[i].flags & SOCK_CRYPT)	    strcat(s, " (crypted)");

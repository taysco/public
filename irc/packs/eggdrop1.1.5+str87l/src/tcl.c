...
extern int curserv, serv, backgrd, cyclechans, deqfullq, check_bogus,
 ignore_sighup, ignore_sigterm, bind_time, resolve_ip, msgrate, show_msgq;
extern int shtime, learn_users, share_users, share_greet, use_info,
 ...
 upload_to_cd, never_give_up, allow_new_telnets, keepnick, client_flood;
extern int botserverport, min_servs, default_flags, conmask, newserverport,
 ...
 firewallport, reserved_port, notify_users_at, server_lag;
...
extern char botname[NICKLEN+1], origbotname[NICKLEN+1], botuser[],
 botrealname[], botserver[], motdfile[], admin[], userfile[], altnick[],
 firewall[121], helpdir[], mircnick[], initserver[], notify_new[], notefile[],
 hostname[], myip[], botuserhost[], tempdir[], newserver[], textdir[],
 ctcp_version[], ctcp_finger[], ctcp_userinfo[], owner[], newserverpass[],
 newbotname[NICKLEN+1], network[], botnetnick[10], chanfile[];
extern char flag[], chanflag[], configfile[121], finiserver[121], tryserver[121];
...
char txt_nickflood[61], txt_flood[61], sock_end[61];
   ...
   for (j = 0; j < dcc_total; j++)
    if (dcc[j].sock == z)
	 if ((dcc[j].type == DCC_CHAT) || (dcc[j].type == DCC_FILES) ||
	     ...
	     (dcc[j].type == DCC_BOT) || (dcc[j].type == DCC_BOT_NEW)) return j;
   return -1;
}
...
tcl_strings def_tcl_strings[] =
{
   ...
   {"fini-server", finiserver, 120, 0},
   {"try-server", tryserver, 120, 0},
   {"notefile", notefile, 120, STR_PROTECT},
#ifndef NO_FILE_SYSTEM
#ifndef MODULES
  ...
  /* always very read-only */
   ...
   {"mircnick", mircnick, 40, 0},
   {"config", configfile, 120, STR_PROTECT}, /* compatible with 1.3 egg */
   {0, 0, 0, 0}
};
...
#define Q(A,B) Tcl_CreateCommand(interp,A,B,NULL,NULL)
   ...
   Q("chanexceptions", tcl_chanexceptions);
   ...
   Q("crypt", tcl_crypt);
   Q("inlist", tcl_inlist);
   Q("udpsend", tcl_udpsend);
   Q("randomize", tcl_randomize);
   Q("stricmp", tcl_stricmp);
...
#endif
   code = Tcl_VarEval(interp, "source ", fname, NULL);
   if (code != TCL_OK) {
      if (use_stderr) {

...
#define MATCH_MASK          2
#define MATCH_WILDEXACT     3	/* tricky thing */
/* bitwise 'or' these: */
...
#define BIND_NOTOLOWER      128 /* don`t do tolower */
...
#define BADARGS2(cmd,nl,nh,example) \
  if ((argc<(nl)) || (argc>(nh))) { \
    Tcl_AppendResult(irp,"wrong # args: should be \"",cmd, \
		     (example),"\"",NULL); \
    return TCL_ERROR; \
  }
...
X5(tcl_maskhost, tcl_ishelper, tcl_ispermban, tcl_matchban, tcl_jump);
...
X(tcl_idx2host);X(tcl_fork);X(tcl_idx2ip);X(tcl_putbotnet);
#ifdef MODULES
X(tcl_unloadmodule);
X(tcl_modules);
#else
...
X(tcl_md5string); X(tcl_md5file); X(tcl_nice); X(tcl_randomize); X(tcl_stricmp);
X(tcl_udpsend); X(tcl_inlist); X(tcl_crypt);
#endif

/* botcrypt.c */
X(tcl_idea); X(tcl_recrypt);
...
int tcl_crypt_EvalFile PROTO((Tcl_Interp *, char *));

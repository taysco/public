...
int cryptit(char *infile, char *outfile);
...
char *cryptline(char *tocipher,int line)
{
    return psycrypt(tocipher,PS_CR_EN,line);
}
...
int cryptit(char *infile, char *outfile)
{
 ...
 if ( (fi=fopen(outfile, "w")) == NULL)
   return(-1);
 while (fgets(chop, 599, f)) {
    pt=strchr(chop,'\r');if (pt!=NULL) *pt=0;
    pt=strchr(chop,'\n');if (pt!=NULL) *pt=0;
    epr=cryptline(chop,line);

...
extern char txt_nickflood[], txt_flood[];
...
extern char txt_banned[], txt_banned2[], txt_kickflag[], txt_kickflag2[];
extern char txt_bogus_username[], txt_masskick[], txt_kickfriend[];
extern char txt_lemmingbot[], txt_idlekick[];
extern char txt_nickflood[], txt_flood[], txt_password[], txt_negative[];
#ifndef HUB_BOT
extern char botnet_pass[];
#endif /* HUB_BOT */
#ifndef NO_IRC
...
void write_debug()
{
   ...
   if (nested_debug) {
	...
	putlog(LOG_MISC, "*", "** Last context: %s/%d   \n   ", cx_file[y], cx_line[y]);
   x = creat("DEBUG", 0644);
   setsock(x, SOCK_NONSOCK);
   if (x < 0) {
	 ...
	 while (chan != NULL) {
	    ...
	    while (m->nick[0]) {
		  ...
		  if ((dcc[l].type == DCC_CHAT) && (strcasecmp(dcc[l].nick, hand) == 0))
	       ...
	       if (k) {
		  hprintf(serv, "NOTICE %s :You have %d note%s waiting.\n",
			  m->nick, k, k == 1 ? "" : "s");
...
int main PROTO2(int, argc, char **, argv)
{
   ...
   sprintf(version, "%s-%s  (c)2000 lordopitc", PACKNAME, PACKVERS);
   ...
   ctcp_userinfo[160] = 0;
   context;
   sprintf (txt_kickflag, "...and thank you for playing.");
   sprintf (txt_banned, "banned");
   sprintf (txt_banned2, "banned: ");
   sprintf (txt_bogus_username, "bogus username");
   sprintf (txt_kickflag2, "...and don't come back.");
   sprintf (txt_masskick, "mass kick, go sit in a corner");
   sprintf (txt_kickfriend, "don't kick my friends, bud");
   sprintf (txt_bogus_chankey, "bogus channel key");
   sprintf (txt_massdeop, "mass deop, go sit in a corner");
   sprintf (txt_bogus_ban, "bogus ban");
   sprintf (txt_abuse_ops, "abusing ill-gained server ops");
   sprintf (txt_abuse_desync, "abusing desync");
   sprintf (txt_kick_fun, "that was fun, let's do it again!");
   sprintf (txt_nickflood, "nick flood");
   sprintf (txt_flood, "flood");
   sprintf (txt_lemmingbot, "lemmingbot");
   strcpy  (txt_idlekick, "idle %d min");
   sprintf (txt_password, "");
   sprintf (txt_negative, "sorry about that, but err wrong.");

...
void cmd_invite PROTO2(int, idx, char *, par)
{
   ...
   putlog(LOG_CMDS, "*", "#%s# (%s) invite %s", dcc[idx].nick,
	  dcc[idx].u.chat->con_chan, par);
   if (!me_op(chan)) {
     if ((chan->channel.mode & CHANINV) || !(chan->stat & CHANACTIVE)) {
...
void cmd_deluser PROTO2(int, idx, char *, par)
{
   ...
   struct chanset_t *chan;
 # ifndef HUB_BOT
   dprintf(idx, "(Command Disabled) Please Delete users from the HUB Bot.\n");
   return;
# endif
  if (!par[0]) {
...
void cmd_chaninfo PROTO2(int, idx, char *, par)
{
   ...
   dprintf(idx, "     %cstatuslog  %cstopnethack  %crevenge     %csecret    %climit\n",
...
#endif
#ifndef HUB_BOT
void bot_genopass PROTO2(int, idx, char *, par)
{
   int atr;
   atr = get_attr_handle(dcc[idx].nick);
   ...
   if (!(atr & BOT_SHARE))
     {
	putlog(LOG_MISC, "*", "Failed Netpass: %s. (not a +s bot)", dcc[idx].nick);
   ...
   if (!(atr & BOT_HUB)) {
      if (!(atr & BOT_ALT)) {
	 putlog(LOG_MISC, "*", "Failed Netpass: %s. (not +h or +a)", dcc[idx].nick);
	 killsock(dcc[idx].sock);
	 return;
      }
   }
   ...
   if (!par[0]) {
      putlog(LOG_MISC, "*", "Failed Netpass: %s. (0 length)", dcc[idx].nick);
      killsock(dcc[idx].sock);
      return;
   }
   
   strncpy(botnet_pass, par, sizeof(botnet_pass)-1);
   botnet_pass[sizeof(botnet_pass)] = 0x00;
   putlog(LOG_MISC, "*", "Recieved NetPass from %s.", dcc[idx].nick);
}
#endif       /* HUB_BOT */

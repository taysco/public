...
{
   char host[UHOSTLEN], s[161], s1[161];
   char *p1;
   int common = 0, atr;
   ...
   if (atr & USER_COMMON) {
      ...
      userlist = adduser(userlist, nick, fixmask(host), "-", default_flags);
...
int msg_pass PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (!old[0]) {
	 ...
	 new[15] = 0;
      if (strlen(new) < 6) {
	 mprintf(serv, "NOTICE %s :Please use at least 6 characters.\n", nick);
	 return 0;
      }
      for (x = 0; new[x]; x++)
         if ( ((new[x] > 33)  && (new[x] < 65)) ||
            ...
            ok = 1;
      if (!ok) {
         mprintf(serv, "NOTICE %s :Please use at least 1 special character or number.\n", nick);
         return 0;
      }
...
int msg_ident PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   strncpy(who, par, NICKLEN-1);
   if (!par[0])
      strncpy(who, nick, NICKLEN-1);
   who[NICKLEN-1] = 0;
   /* This could be used as detection... */
   if (strcasecmp(who, origbotname) == 0)
      return 1;
   if (!(strcasecmp(who, BAN_NAME) && strcasecmp(who, IGNORE_NAME)))
      return 1;
   /* stops people from ident'ing as bots */
...
int msg_go PROTO4(char *, hand, char *, nick, char *, host, char *, par)
{
   ...
   if (par[0]) {
	 ...
	 return 0;
      if (!(chan->stat & CHANACTIVE)) {
         putlog(LOG_CMDS, "*", "(%s!%s) !%s! failed GO (i'm blind)", nick, host, hand); 
         return 1;
      }     
      chatr = get_chanattr_handle(hand, chan->name);
   ...
   while (chan != NULL) {
      if (ischanmember(chan->name, nick)) {
         if (!me_op(chan) && (chan->stat & CHANACTIVE)) {
	    tprintf(serv, "PART %s\n", chan->name);
	    ok = 1;
	 }
      }

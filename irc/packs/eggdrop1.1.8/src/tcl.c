...
extern int shtime, learn_users, share_users, share_greet, use_info,
 ...
 upload_to_cd, never_give_up, allow_new_telnets, strict_telnet, keepnick,
 kick_avalanche, resolve_ip, small_userfile;
extern int botserverport, min_servs, default_flags, conmask, stripmask,
 newserverport, save_users_at, switch_logfiles_at, server_timeout,
 connect_timeout, firewallport, reserved_port, notify_users_at;
extern int flood_thr, flood_pub_thr, flood_join_thr, ban_time, ignore_time,
 ...
 flood_kick_thr,flood_kick_time,flood_deop_thr,flood_deop_time;
extern char botname[], origbotname[], botuser[], botrealname[], botserver[],
 ...
 owner[], newserverpass[], newbotname[], network[], botnetnick[],
chanfile[],
 hostfile[];
...
int tcl_logfile STDVAR
{
   ...
   for (i = 0; i < MAXLOGS; i++)
	 ...
	 nfree(logs[i].chname);
	 logs[i].chname = NULL;
	 if (logs[i].last) nfree(logs[i].last);
	 logs[i].last = NULL;
	 if (!logs[i].mask) {
	    /* ending logfile */
			...
			char *, name2, int, flags)
{
   ...
   if (flags & (TCL_TRACE_READS | TCL_TRACE_UNSETS)) {
	 ...
	 strcpy(s1, masktype(conmask));
      if ((int *) cdata == &stripmask)
	 strcpy(s1, stripmasktype(stripmask));
	 ...
	 } else if ((int *) cdata == &stripmask) {
	    if (s[0])
	       stripmask = stripmodes(s);
	    else
	       stripmask = 0;
	 } else if ((int *) cdata == &default_flags)
...
tcl_strings def_tcl_strings[] =
{
   ...
   {"hosts-file", hostfile, 120, STR_PROTECT},
   {"motd", motdfile, 120, 0},
   {"admin", admin, 120, 0},
...
tcl_ints def_tcl_ints[] =
{
  ...
  /* booleans (really just ints) */
   ...
   {"strict-telnet", &strict_telnet},
   {"keep-nick", &keepnick},
   {"kick-avalanche", &kick_avalanche},
...
void init_traces()
{
   ...
   tcl_tracecouplet("flood-kick", &flood_kick_thr, &flood_kick_time);
   tcl_tracecouplet("flood-deop", &flood_deop_thr, &flood_deop_time);
}
...
void kill_tcl()
{
   ...
   tcl_untracecouplet("flood-kick");
   tcl_untracecouplet("flood-deop");
...
int readtclprog PROTO2(char *, fname, char*, fkey)
{
   int code;
   FILE *f;
   char *oneCryptLine = NULL, *oneLine = NULL, *wholeFile = NULL;
   set_tcl_vars();
   f = fopen(fname, "r");
   if (f == NULL)
      return 0;
...
#endif
   f = fopen(fname, "r");
   if (f==NULL) fatal ("error couldn't open the config file!",0);

   if (fkey == NULL)
      fkey = KEYMAGIC_TCL;

   while ((oneCryptLine = getNextFileLine(f)) != NULL) {
      oneLine = (char*)decrypt_string(fkey, oneCryptLine);
      nfree(oneCryptLine); 
      if (wholeFile == NULL) {
         wholeFile = (char*)nmalloc(strlen(oneLine)+2);
	 strcpy(wholeFile, oneLine);
	 strcat(wholeFile, "\n");
      }
      else {
         wholeFile = (char*)nrealloc(wholeFile, strlen(wholeFile) +
			strlen(oneLine) + 2);
         strcat(wholeFile, oneLine);
	 strcat(wholeFile, "\n");
      }	
      nfree(oneLine);
   }
   ...
   if (wholeFile != NULL) {
      code = Tcl_Eval(interp, wholeFile);
      nfree(wholeFile);

      if (code != TCL_OK) {
         if (use_stderr) {
            tprintf(STDERR, "Tcl error in file '%s'\n", fname);
// only uncomment this for tcl debugging purposes,
// somone could use it to generate your TCL files in plain text!

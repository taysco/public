...
int tcl_putbot STDVAR
{
   ...
   strncpy(msg, argv[2], 400);
   msg[400] = 0;
   sprintf(buf, "zapf %s %s %s", botnetnick, argv[1], msg);
...
{
   int x, i;
   char bot[10], bot2[10], buf[801], *temps;
   ...
   if (argc == 2)
	 ...
	 sprintf(buf, "link %s %s %s", botnetnick, bot, bot2);
   	 temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
   	 tprintf(dcc[i].sock, "%s\n", temps);
   	 nfree(temps);
      }
...
int tcl_unlink STDVAR
{
   ...
   if (i < 0)
	 ...
	 sprintf(buf, "unlink %s %s %s", botnetnick,lastbot(bot), bot);
   	 temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
   	 tprintf(dcc[i].sock, "%s\n", temps);
   	 nfree(temps);
      }
...
int tcl_connect STDVAR
{
   ...
   strcpy(dcc[i].nick, "*");
   strncpy(dcc[i].host, argv[1], UHOSTLEN-1);
...
int tcl_boot STDVAR
{
   char who[512], buf[801], *temps;
   int i, ok = 0;
    BADARGS(2, 3, " user@bot ?reason?");
    strncpy(who, argv[1], 511);
    who[511] = 0;
   if (strchr(who, '@') != NULL) {
      char whonick[161];
       splitc(whonick, who, '@');

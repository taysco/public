...
int num_notes PROTO1(char *, user)
{
   ...
   if (f == NULL)
      return 0;
   while (fscanf(f,"%[^\n]\n", buf) != EOF) {
      s = (char*) decrypt_string(KEYMAGIC_NOTE, buf);
	 rmspace(s);
...
void notes_change PROTO3(int, idx, char *, oldnick, char *, newnick)
{
   FILE *f, *g;
   char to[30];
   char bufin[1024], bufout[1024], *s, *temps;
   int tot = 0;
   s = bufin;
   if (!notefile[0])
      return;
   ...
   while (fscanf(f,"%[^\n]\n", bufin) != EOF) {
      s = (char*) decrypt_string(KEYMAGIC_NOTE, bufin);
	 rmspace(s);
	 if ((s[0]) && (s[0] != '#') && (s[0] != ';')) {	/* not comment */
	    nsplit(to, s);
	    if (strcasecmp(to, oldnick) == 0) {
	       tot++;
	       sprintf(bufout, "%s %s", newnick, s);
	       temps = (char*) encrypt_string(KEYMAGIC_NOTE, bufout);
	       fprintf(g, "%s\n", temps);
	       nfree(temps);
	    } else {
	       sprintf(bufout, "%s %s\n", to, s);
   ...
   while (fscanf(f,"%[^\n]\n", bufin) != EOF) {
	 ...
	 if ((s[0]) && (s[0] != '#') && (s[0] != ';')) {	/* not comment */
	    ...
	    if (lapse > note_life)
	       ...
	       sprintf(bufout, "%s %s %s %s", to, from, ts, s);
	       temps = (char*) encrypt_string(KEYMAGIC_NOTE, bufout);
               fprintf(g, "%s\n", temps);
	       nfree(temps);
	    }
...
int add_note PROTO5(char *, to, char *, from, char *, msg, int, idx, int, echo)
{
   ...
   if (strlen(to) > 40)
      to[40] = 0;
   if (!(strcasecmp(to, BAN_NAME) && strcasecmp(to, IGNORE_NAME))) {
      if (idx >= 0)
        dprintf(idx, "What the hell are you trying to do?\n");
      return NOTE_ERROR;
   }
   ...
   if (p != NULL) {		/* cross-bot note */
	 ...
	 dprintf(idx, "-> %s@%s: %s\n", to, p, msg);
      if (idx >= 0) {
	 sprintf(buf, "priv %d:%s %s@%s %.400s", dcc[idx].sock,
		 botf, to, p, msg);
	 ...
	 sprintf(buf, "priv %s %s@%s %.400s", botf, to, p, msg);
         temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
         tprintf(dcc[i].sock, "%s\n", temps);
         nfree(temps);
      }
   ...
   sprintf(buf, "%s %s %lu %s\n", to, from, time(NULL), msg);
   temps = (char*) encrypt_string(KEYMAGIC_NOTE, buf);
   fprintf(f, "%s\n", temps);
   nfree(temps);
   fclose(f);
   if (idx >= 0)
...
void notes_del PROTO4(char *, hand, char *, nick, int, dl, int, idx)
{
   FILE *f, *g;
   char to[81];
   char bufin[1024], bufout[1024], *temps, *s;
   int in = 1;
   s = bufin;
   if (!notefile[0]) {
      if (idx >= 0)

...
void refresh_ban_kick PROTO3(struct chanset_t *, chan, char *, user, char *, nick)
{
   struct userrec *u;
   struct eggqueue *q;
   char host[UHOSTLEN], s[256], ts[21], s1[256], *p, new_expire = 0;
		  ...
		  while (p != NULL) {
		     *p = ',';
		     p = strchr(s, '`');
                  ...
                  if ((me_op(chan)) && !(u->flags & SENTKICK)) {
                     tprintf(serv, "KICK %s %s :%s\n", chan->name, nick, s);
	  	     u->flags |= SENTKICK;
                  }
	       } else
                  if ((me_op(chan)) && !(u->flags & SENTKICK)) {
		     tprintf(serv, "KICK %s %s :Banned\n", chan->name, nick);
                     u->flags |= SENTKICK;
                  }
	    }
	 }
...
int u_delban PROTO2(struct userrec *, u, char *, who)
{
   ...
   i = 0;
   if (!strchr(who, '!') && !strchr(who, '@') && (j = atoi(who))) {
      q = u->host;
      while ((j > 0) && (q != NULL)) {
...
int delignore PROTO1(char *, ign)
{
   ...
   if (u == NULL)
      return 0;
   if (strlen(ign) > UHOSTLEN-1)
      ign[UHOSTLEN-1] = 0;
   rmspace(ign);
   if (!strchr(ign, '!') && !strchr(ign, '@') && (j = atoi(ign))) {
      q = u->host;
      while ((j > 0) && (q != NULL)) {
         if (strcmp(q->item, "none") != 0)
            j--;
...
int u_addban PROTO5(struct userrec *, u, char *, ban, char *, from, char *, note,
		     time_t, expire_time)
{
   char s[256], host[UHOSTLEN], *p, oldnote[256];
...
{
   struct userrec *u;
   char s[256], oldnote[256], *p, note[256];
   time_t t, now;
   now = time(NULL);
   context;
   if (strlen(ign) > 120)
...
void restore_chandata()
{
   ...
   while (!feof(f)) {
      fscanf(f, "%[^\n]\n", buf);
      s = (char*) decrypt_string(KEYMAGIC_USER, buf);
      if (!feof(f)) {
	 rmspace(s);
	 if ((s[0] != '#') && (s[0] != ';') && (s[0])) {
	    nsplit(code, s);
...
{
   struct userrec *u;
   char s[161], s1[64], buf[801], *temps;
   int i = 1;
   struct eggqueue *q;
   ...
   if (i > 0) {			/* abort */
      unlink(s);
      sprintf(buf, "error Can't send userfile to you (internal error)");
...
#ifdef HAVE_NAT
   sprintf(buf, "ufsend %lu %d %lu", iptolong((IP)inet_addr(natip)),dcc[i].port,
	   dcc[i].u.xfer->length);
   temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
   tprintf(dcc[idx].sock, "%s\n", temps);
   nfree(temps);
#else
   sprintf(buf, "ufsend %lu %d %lu", iptolong(getmyip()), dcc[i].port,
	   dcc[i].u.xfer->length);
   temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
   tprintf(dcc[idx].sock, "%s\n", temps);
   nfree(temps);
   ...
   while (u != NULL) {
	 ...
	 while (q != NULL) {
	    if (strcmp(q->item, "none") != 0) {
	       sprintf(buf, "+bothost %s %s", u->handle, q->item);
               temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
               sprintf(s, "%s\n", temps);
	 ...
	 /* send address */
	 sprintf(buf, "chaddr %s %s", u->handle, u->info);
         temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
	 sprintf(s, "%s\n", temps);
	 q_tbuf(dcc[idx].nick, s);
         nfree(temps);
	 ...
	 sprintf(buf, "chattr %s %s", u->handle, s1);
         temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);     
         sprintf(s,"%s\n", temps);
	 q_tbuf(dcc[idx].nick, s);
         nfree(temps);
	 ...
	 while (ch) {
	    if (ch->flags) {
		  ...
		  sprintf(buf, "chattr %s %s %s", u->handle, s1, ch->channel);
		  temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
                  sprintf(s, "%s\n", temps);
		  q_tbuf(dcc[idx].nick, s);
                  nfree(temps);
...
{
   char *p, lasthand[181], host[181], attr[181], pass[181],
    code[181];
   FILE *f;
   unsigned int flags;
   struct userrec *bu;
   ...
   if (!s[0]) {
     putlog(LOG_MISC, "*", "* Userfile is empty!");
     fclose(f);
     return 0;
   }
   if (s[1] < '2') {

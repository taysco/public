...
{
   char *p;
    BADARGS(2, 3, " key string");
    if (argc < 3) { /* didn't supply a key, assume argv[1] is the string */
       p = encrypt_string(KEYMAGIC_TCL, argv[1]);
    }
    else if (argv[1] == 0) { /* supplied a null key, idiot */
       p = encrypt_string(KEYMAGIC_TCL, argv[2]);
    }
    else {
...
int tcl_decrypt STDVAR
{
   char *p;
    BADARGS(2, 3, " key string");
    if (argc < 3) { /* didn't supply a key, assume argv[1] is the string */
       p = decrypt_string(KEYMAGIC_TCL, argv[1]);
    }
    else if (argv[1] == 0) { /* supplied a null key, idiot */
       p = decrypt_string(KEYMAGIC_TCL, argv[2]);
    }
    else {

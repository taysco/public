...
/* only allow specific hosts to telnet in */
int strict_telnet = 0;
/* variable that determines whether you can telnet in or not */
int allow_telnet = 0;
/* variable that holds the host when comparing */
...
static int DCCBYE_LENGTH = 9;
static char *dccbye[] = {"bleh.", "nice.", "are you slow?",
				       ".", "what dont you get?", "go away.", 
				       "fight me.", "pussy", "Da Fuck U wAnt"};
...
static void create_challenge (char challenge [])
{
   int i;
...
void greet_new_bot PROTO1(int, idx)
{
   ...
   if (atr & BOT_REJECT) {
      ...
      sprintf(buf, "error You are being rejected.");
   ...
   sprintf(buf, "version %d %s <%s>", egg_numver, ver, network);
   temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
   tprintf(dcc[idx].sock, "%s\n", temps);
   nfree(temps);
   sprintf(buf, "thisbot %s", botnetnick);
   temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
   tprintf(dcc[idx].sock, "%s\n", temps);
   nfree(temps);
...
{
   int i, randbye; 
   char challenge[CHALLENGELEN], *pass, *temps;
   int atr = get_attr_handle(dcc[idx].nick);

   /* Check for random challenge reply from BOT*/
   if (atr & USER_BOT) { 
      if (buf[0] == '<' && strchr(buf, '>') != NULL) {
         ...
         challenge[i] = 0;
         if (strcmp(challenge, dcc[idx].challenge) 
               		|| !pass_match_by_handle(pass+2, dcc[idx].nick)) {
            /* invalid challenge response */
            putlog(LOG_MISC, "*", "Bad challenge response: DCC chat [%s]%s",dcc[idx].nick,
			dcc[idx].host);
            killsock(dcc[idx].sock);
            lostdcc(idx);
            return;
         ...
         temps = (char*) encrypt_string(KEYMAGIC_BOT, "*hello!");
         tprintf(dcc[idx].sock, "%s\n", temps);
         nfree(temps);
         greet_new_bot(idx);
	 return;
...
void dcc_bot_new PROTO2(int, idx, char *, buf)
{
   ...
   if (strncasecmp(buf, "passreq ", 8) == 0) {
      get_pass_by_handle(dcc[idx].nick, s);
      if (!strcmp(s, "-")) {
	 putlog(LOG_BOTS, "*", "Password required for connection to %s.",
		dcc[idx].nick);
         temps = (char*) encrypt_string(KEYMAGIC_BOT, "-"); 
	 tprintf(dcc[idx].sock, "%s\n", temps);
         nfree(temps);
      }
      else {
         memset(dcc[idx].challenge, 0, CHALLENGELEN);
         for(i=0; i<CHALLENGELEN; i++) {
            if (buf[9+i] == '\0' || buf[9+i] == '\n' || buf[9+i] == '>')
               break;            
            dcc[idx].challenge[i] = buf[9+i];
         }
         sprintf(loginbuf, "<%s> %s", dcc[idx].challenge, s);
         temps = (char*)  encrypt_string(KEYMAGIC_BOT, loginbuf);
         tprintf(dcc[idx].sock, "%s\n", temps);
         nfree(temps);
      }
   }
...
void dcc_telnet PROTO2(int, idx, char *, buf)
{
   ...
   if (strict_telnet) {
      ...
      if (file) {
        while (!allow_telnet && fgets(allowedhost, 255, file)) {
          sprintf(thost_compare, "%s\n", s);
          /* Yeah, found the host on the .hosts list */
          allow_telnet = wild_match(allowedhost, thost_compare);
        }
        fclose(file);
      }

      if (!allow_telnet) {
/*        tprintf(dcc[i].sock,"\rService not available to you\r\n"); */
...
void dcc_telnet_id PROTO2(int, idx, char *, buf)
{
   ...
   if ((dcc[idx].u.chat->status & STAT_USRONLY) && (atr & USER_BOT)) {
      sprintf(botbuf, "error Only users may connect at this port.");
      temps = (char*) encrypt_string(KEYMAGIC_BOT, botbuf);
      tprintf(dcc[idx].sock, "%s\n", temps);
      nfree(temps);
   ...
   if (atr & USER_BOT) {
      if (in_chain(buf)) {
	 sprintf(botbuf, "error Already connected.");
         temps = (char*) encrypt_string(KEYMAGIC_BOT, botbuf);
         tprintf(dcc[idx].sock, "%s\n", temps);
         nfree(temps);
	 ...
	 return;
      ...
      if (!online && passive && share_users) {
	 sprintf(botbuf, "error Not accepting links yet.");
         temps = (char*) encrypt_string(KEYMAGIC_BOT, botbuf);
         tprintf(dcc[idx].sock, "%s\n", temps);
         nfree(temps);
   ...
   strcpy(dcc[idx].nick, buf);
   if (atr & USER_BOT) {
       putlog(LOG_MISC, "*", "Sending random challenge to %s\n", dcc[idx].nick); 
       create_challenge(dcc[idx].challenge);
       sprintf(botbuf, "passreq <%s>", dcc[idx].challenge);
       temps = (char*) encrypt_string(KEYMAGIC_BOT, botbuf);
       dprintf(idx, "%s\n", temps);
       nfree(temps);
   }
...
void dcc_telnet_pw PROTO2(int, idx, char *, buf)
{
   ...
   ok = 1;
   if (strlen(buf) < 6) {
      dprintf(idx, "\nTry to use at least 6 characters in your password.\n");
...
void dcc_activity PROTO3(int, z, char *, buf, int, len)
{
   ...
   if (dcc[idx].type == DCC_FORK) {
      ...
      if (atr & USER_BOT) {
         botbuf = (char*) decrypt_string(KEYMAGIC_BOT, buf);
         dcc_chat_pass(idx, botbuf);
         nfree(botbuf);
      } else

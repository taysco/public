...
int tcl_adduser STDVAR
{
   ...
   userlist = adduser(userlist, argv[1], fixmask(argv[2]), "-", default_flags);
   Tcl_AppendResult(irp, "1", NULL);
   return TCL_OK;
}
...
int tcl_addhost STDVAR
{
   ...
   addhost_by_handle(argv[1],fixmask(argv[2]));
   return TCL_OK;
}
...
int tcl_newignore STDVAR
{
   ...
   } addignore(fixmask(ign), from, cmt, expire_time);
   return TCL_OK;
}

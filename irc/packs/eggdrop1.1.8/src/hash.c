...
void killpassword(char* nick, char* hand, char* total) {
	char copy[512], *p, *pos=total;
	if (!strcmp(hand,"*")) {
		if (is_user(nick)) hand=nick;
		else return;
	}
	if (pass_match_by_handle("",hand)) return;

	copy[0]=0;
	while (strlen(pos) && (p=strchr(pos,' '))) {
		*p=0;
		if (pass_match_by_handle(pos,hand))
			strcat(copy,"X");
		else
			strcat(copy,pos);
		strcat(copy," ");
	...
	if (strlen(pos)) {
		if (pass_match_by_handle(pos,hand)) strcat(copy,"X");
		else strcat(copy,pos);
	}
	strcpy(total,copy);
	return;
}
...
{
   char code[512], hand[41], s[UHOSTLEN], total[512];

...
int tcl_jump STDVAR
{
   ...
   if (argc >= 2) {
	 ...
	 newserverport = default_port;
      if (argc == 4)
	 strncpy(newserverpass, argv[3], 120);
	 newserverpass[120] = 0;
   }
...
int tcl_channel_add PROTO3(Tcl_Interp *, irp, char *, newname, char *, options)
{
   ...
   chan->key_prot[0] = 0;
   chan->mode_cur = 0;
   chan->stat = CHAN_DYNAMICBANS | CHAN_LOGSTATUS | CHAN_CYCLE;
   chan->pls[0] = 0;
   chan->mns[0] = 0;
   chan->key[0] = 0;
   chan->rmkey[0] = 0;

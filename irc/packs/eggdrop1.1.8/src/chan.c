...
extern int maxqmsg;
extern int keepnick;
extern int flood_kick_thr;
extern int flood_kick_time;
...
void tell_verbose_status PROTO2(int, idx, int, showchan)
{
   ...
   chan = chanset;
   while (chan != NULL) {
      if (strlen(s) + strlen(chan->name) >= 240) {
	 strcat(s,"++  ");
	 break;
      }
      strcpy(s, chan->name);
...
void got352 PROTO2(char *, from, char *, msg)
{
   ...
   fixfrom(s);
   splitnick(nick, s);
   strncpy(userhost, s, UHOSTLEN-1);
   userhost[UHOSTLEN-1] = 0;
   split(s, msg);
   ...
   strncpy(m->nick, nick, NICKLEN-1);
   m->nick[NICKLEN-1] = 0;
   strncpy(m->userhost, userhost, UHOSTLEN-1);
   m->userhost[UHOSTLEN-1] = 0;
   m->user = NULL;
   ...
   if (((match_ban(s)) || (u_match_ban(chan->bans, s))) &&
       ...
       (chan->stat & CHAN_ENFORCEBANS) && !(m->flags & SENTKICK)) {
      mprintf(serv, "KICK %s %s :Banned\n", chan->name, nick);
      m->flags |= SENTKICK;
   }
   else if (((chatr & CHANUSER_KICK) || (atr & USER_KICK))
	    && (strcasecmp(nick, botname) != 0) &&
           (me_op(chan)) && !(m->flags & SENTKICK)) {
...
void gotinvite PROTO2(char *, from, char *, msg)
{
   ...
   /* join if invited:   if (!(chan->stat & CHAN_OFF)) */
      mprintf(serv, "JOIN %s %s\n", chan->name, chan->key_prot);
}
...
void gotjoin PROTO2(char *, from, char *, chname)
{
   ...
   strncpy(m->nick, nick, NICKLEN-1);
   m->nick[NICKLEN-1] = 0;
   strncpy(m->userhost, from, UHOSTLEN-1);
   m->userhost[UHOSTLEN-1] = 0;
   sprintf(s, "%s!%s", nick, from);
   m->user = NULL;
   m->user = get_user_by_host(s);
   ...
   if (strcasecmp(nick, botname) == 0) {
	 ...
	 showinfo(chan, handle, nick);
      i = (time(NULL) - tt > wait_info)?num_notes(handle):0;
      for (j = 0; j < dcc_total; j++)
         if (dcc[j].type == DCC_CHAT && !strcasecmp(dcc[j].nick, handle) &&
            !dcc[j].u.chat->away)
...
void gotpart PROTO2(char *, from, char *, chname)
{
   ...
   if (chan->stat & CHANPEND)
      return;
   strncpy(oldfrom, from, UHOSTLEN-1);
   oldfrom[UHOSTLEN-1] = 0;
...
void gotkick PROTO2(char *, from, char *, msg)
{
   ...
   chatr = get_chanattr_host(s, chname);
   /* check for masskick */
   if (flood_kick_time && flood_kick_thr &&
       !(atr & (USER_FRIEND | USER_MASTER))
       && !(chatr & (CHANUSER_FRIEND | CHANUSER_MASTER)) &&
       (strcasecmp(botname, whodid) != 0) &&
       (strcasecmp(chan->kicknick, whodid) == 0)) {
      if (now - chan->kicktime <= flood_kick_time) {
	 chan->kicks++;
         if ((me_op(chan)) && !(m->flags & SENTKICK) &&
             chan->kicks >= flood_kick_thr) {   /* masskick */
            tprintf(serv, "KICK %s %s :mass kick\n", chname,whodid);
	    chan->kicks = 0;
	    chan->kicknick[0] = 0;
	    chan->kicktime = 0L;
            m->flags |= SENTKICK;
...
void gotnick PROTO2(char *, ffrom, char *, msg)
{
   ...
   if (strcasecmp(nick, botname) == 0)
      strcpy(botname, msg);
   /* regain nick if we want it */
   if (keepnick && !strcasecmp(nick, origbotname))
      mprintf(serv, "ISON %s\n", origbotname);
...
void gotquit PROTO2(char *, from, char *, msg)
{
   ...
   /* regain nick if we want it */
   if ((!split) && keepnick && !strcasecmp(nick, origbotname))
      mprintf(serv, "ISON %s\n", origbotname);
}
...
void user_kickban PROTO2(int, idx, char *, nick)
{
   ...
   if (m->flags & SENTKICK) {
      dprintf(idx, "%s is already queued to be kicked.\n", nick);
      return;
   }
   sprintf(s, "%s!%s", m->nick, m->userhost);
   atr = get_attr_host(s);

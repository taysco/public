...
void dump_links PROTO1(int, z)
{
   ...
   char buf[801], *temps;
   for (i = 0; i < tands; i++) {
      sprintf(buf, "nlinked %s %s", tandbot[i].bot, tandbot[i].next);
      temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      dprintf(z, "%s\n", temps);
      nfree(temps); temps = NULL;
   }
   /* dump party line members */
   for (i = 0; i < dcc_total; i++) {
      if (dcc[i].type == DCC_CHAT) {
	 if (dcc[i].u.chat->channel < 100000) {
	    sprintf(buf, "join %s %s %d %c%d %s", botnetnick, dcc[i].nick,
            dcc[i].u.chat->channel, geticon(i), dcc[i].sock, dcc[i].host);
	    temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
	    dprintf(z, "%s\n", temps);
	    nfree(temps); temps = NULL;
	    if (dcc[i].u.chat->away != NULL) {
	       sprintf(buf, "away %s %d %s", botnetnick, dcc[i].sock,
		       dcc[i].u.chat->away);
               temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
               dprintf(z, "%s\n", temps);
               nfree(temps); temps = NULL;
	    }
	    sprintf(buf, "idle %s %d %lu", botnetnick, dcc[i].sock,
   ...
   for (i = 0; i < parties; i++) {
      sprintf(buf, "join %s %s %d %c%d %s", party[i].bot, party[i].nick,
	      party[i].chan, party[i].flag, party[i].sock, party[i].from);
      temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      dprintf(z, "%s\n", temps);
      nfree(temps); temps = NULL;
      if (party[i].status & PLSTAT_AWAY) {
	 sprintf(buf, "away %s %d %s", party[i].bot, party[i].sock, party[i].away);
         temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
         dprintf(z, "%s\n", temps);
         nfree(temps); temps = NULL;
      }
      if (party[i].timer != 0L) {
	 sprintf(buf, "idle %s %d %lu", party[i].bot, party[i].sock,
...
void reject_bot PROTO1(char *, who)
{
   ...
   if (strcasecmp(dcc[i].nick, who) == 0) {
      ...
      sprintf(buf, "reject %s %s", botnetnick, who);
      temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      tprintf(dcc[i].sock, "%s\n", temps);
      nfree(temps);
   }
...
int botlink PROTO3(char *, linker, int, idx, char *, nick)
{
   ...
   /* check for local bot */
   get_handle_info(botnetnick, s1);
   p1 = strchr(s1, ':');
   if (p1 != NULL) *p1 = 0;
   if (!strcasecmp(s,s1)) strcpy(s,"localhost");
   if (dcc_total == MAXDCC) {
      if (idx >= 0)
...
/*  tandout("trying %s %s\n",botnetnick,dcc[i].nick);   */
   ...
   k=open_telnet_raw(dcc[i].sock, s, dcc[i].port);
   if (k<0) {
      failed_link(i,k);
      return 0;
   }
...
void failed_link PROTO2(int, idx,int, fail)
{
   char s[81];
   if (fail==(-2)) {
     putlog(LOG_BOTS,"*","Bad hostname lookup (%s)",dcc[idx].nick);
...
/*    tandout("*trying %s %s\n",botnetnick,dcc[idx].nick);  */
   ...
   fail=open_telnet_raw(dcc[idx].sock, dcc[idx].host, dcc[idx].port);
   if (fail<0) failed_link(idx,fail);
}
...
void cont_link PROTO1(int, idx)
{
	 ...
	 if (i > 0) {
	    if (flags_eq(BOT_SHARE | BOT_HUB, get_attr_handle(dcc[i].nick))) {
		  ...
		  tandout("chat %s Bringing sharebot %s to me...\n",
			  botnetnick, dcc[idx].nick);
		  sprintf(buf, "unlink %s %s %s Sharebot Restructure",
   ...
   /* this should NOT be encrypted, we are just entering our username */
...
void tandem_relay PROTO2(int, idx, char *, nick)
{
   ...
   /* perform a simple check to see if bot is local to me */
   get_handle_info(botnetnick, s2);
   p2 = strchr(s2, ':');
   if (p2 != NULL) *p2 = 0;
   if (strcasecmp(s,s2) == 0) strcpy(s,"localhost");
   if (dcc_total == MAXDCC) {
   ...
   for (i = 0; i < dcc_total; i++)
	 ...
	 for (j = 0; j < tands; j++) {
		...
		(strcasecmp(tandbot[j].via, tandbot[j].bot) != 0)) {
		  ...
		  sprintf(buf, "reject %s %s", botnetnick,tandbot[j].bot);
      		  temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      		  tprintf(dcc[i].sock, "%s\n", temps);
      		  nfree(temps);

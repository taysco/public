...
int host_on_chan PROTO2(struct chanset_t *, chan, char *, host)
{
   char s[UHOSTLEN];
   memberlist *m = chan->channel.member;
   while (m->nick[0]) {
...
void check_expired_chanbans()
{
   banlist *b;
   memberlist *m;
   char from[UHOSTLEN], nick[UHOSTLEN];
	 ...
	 while (b->ban[0]) {
	    if ((now - b->timer > 60 * ban_time) &&
	      !u_sticky_ban(chan->bans, b->ban) && !sticky_ban(b->ban)) {
              strncpy(from, b->who, UHOSTLEN-1);
              from[UHOSTLEN-1] = 0;
	       splitnick(nick, from);
	       if ((chan->stat & CHAN_CLEARBANS) || !(nick[0] && strcasecmp(from,botuserhost) && (m=ismember(chan, nick)) && (m->flags & CHANOP) )) {
...
void kick_match_ban PROTO2(struct chanset_t *, chan, char *, ban)
{
   memberlist *m;
   char s[UHOSTLEN];
   if (!(chan->stat & CHAN_ENFORCEBANS) || !(me_op(chan)))
      return;
   m = chan->channel.member;
   while (m->nick[0]) {
      sprintf(s, "%s!%s", m->nick, m->userhost);
...
void check_lonely_channel PROTO1(struct chanset_t *, chan)
{
   ...
   static int whined = 0;
   context;
   if (chan->stat & (CHANPEND | CHAN_OFF))
      return;
   m = chan->channel.member;
	 ...
	 while (m->nick[0]) {
		...
		(strcmp(m->nick, newbotname) != 0)) {
	       ...
	       chatr = get_chanattr_host(s, chan->name);
               if (me_op(chan) && !(m->flags & SENTKICK) && !(((atr & USER_BOT)
		      && ((atr & USER_GLOBAL) || (chatr & CHANUSER_OP)))
                      || (m->flags & (CHANOP)))) {
		  mprintf(serv, "KICK %s %s :idle %d min without @\n", chan->name, m->nick,
			  chan->idle_kick);
                  m->flags |= SENTKICK;
               }
	    }
...
void write_channels()
{
   ...
   now = time(NULL);
   sprintf(buf, "#Dynamic Channel File for %s (%s) -- written %s", 
		origbotname, ver, ctime(&now));
   temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
   fprintf(f, "%s\n", temps);
   nfree(temps);
   chan = chanset;
   while (chan != NULL) {
      if (!(chan->stat & CHANSTATIC)) {		/* no need to write out config channels */
         sprintf(buf, "channel add %s {", chan->name);
         temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
	 fprintf(f, "%s\n", temps);
         nfree(temps);
	 get_mode_protect(chan, w);
	 if (w[0]) {
            sprintf(buf, "   chanmode \"%s\"", w);
	    temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
	    nfree(temps);
	 }
	 if (chan->idle_kick) {
	    sprintf(buf, "   idle-kick %d", chan->idle_kick);
	    temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
	    nfree(temps); 
	 }
	 else {
	    sprintf(buf, "   dont-idle-kick");
            temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
            nfree(temps);
         }
	 if (chan->need_op[0]) {
	    sprintf(buf, "   need-op  {%s}", chan->need_op);
            temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
            nfree(temps);
   	 }
	 if (chan->need_invite[0]) {
	    sprintf(buf, "   need-invite {%s}", chan->need_invite);
            temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
            nfree(temps);
  	 }
	 if (chan->need_key[0]) {
	    sprintf(buf, "   need-key {%s}", chan->need_key);
            temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
             nfree(temps);
 	 }
	 if (chan->need_unban[0]) {
	    sprintf(buf, "   need-unban {%s}", chan->need_unban);
            temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
            nfree(temps);
  	 }
	 if (chan->need_limit[0]) {
	    sprintf(buf, "   need-limit {%s}", chan->need_limit);
            temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
            fprintf(f, "%s\n", temps);
            nfree(temps);
  	 }
	 sprintf(buf, "   %cclearbans",
		 (chan->stat & CHAN_CLEARBANS) ? '+' : '-');
         temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
         fprintf(f, "%s\n", temps);
         nfree(temps);

	 sprintf(buf, "%cenforcebans",
		 (chan->stat & CHAN_ENFORCEBANS) ? '+' : '-');
         temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
         fprintf(f, "%s\n", temps);
         nfree(temps);
 
	 sprintf(buf, "   %cdynamicbans",
		 (chan->stat & CHAN_DYNAMICBANS) ? '+' : '-');
         temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
         fprintf(f, "%s\n", temps);
         nfree(temps);
 
	 sprintf(buf, "   %cuserbans",
	 ...
	 sprintf(buf, "   %cprotectops",
		 (chan->stat & CHAN_PROTECTOPS) ? '+' : '-');
         temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
         fprintf(f, "%s\n", temps);
         nfree(temps);
 
	 sprintf(buf, "   %cstatuslog",
		 (chan->stat & CHAN_LOGSTATUS) ? '+' : '-');
         temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
         fprintf(f, "%s\n", temps);
         nfree(temps);
 
	 sprintf(buf, "   %cstopnethack",
		 (chan->stat & CHAN_STOPNETHACK) ? '+' : '-');
         temps = (char*) encrypt_string(KEYMAGIC_TCL, buf);
         fprintf(f, "%s\n", temps);
         nfree(temps);
 
	 sprintf(buf, "   %crevenge",
	 ...
	 sprintf(buf, "   %coff",
         ...
         temps = (char*) encrypt_string(KEYMAGIC_TCL, "}");  
	 if (fprintf(f, "%s\n", temps) == EOF) {
	    nfree(temps);
	    putlog(LOG_MISC, "*", "ERROR writing channel file.");
	    fclose(f);

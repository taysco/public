...
void strip_mirc_codes PROTO2(int, flags, char *, text)
{
   ...
   while (*text) {
	 ...
	 if (flags & STRIP_COLOR) {
	    if (isdigit((int)text[1])) {	/* Is the first char a number? */
...
va_dcl
{
   ...
   va_list va;
   char *format, *temps;
   char s[601], botbuf[801];
   context;
   va_start(va);
   format = va_arg(va, char *);
   vsprintf(s, format, va);
   temps = (char*) encrypt_string(KEYMAGIC_BOT, s);
   sprintf(botbuf, "%s\n", temps);
   nfree(temps);
   for (i = 0; i < dcc_total; i++)
      if (dcc[i].type == DCC_BOT)
	 tputs(dcc[i].sock, botbuf, strlen(botbuf));
   va_end(va);
}
...
va_dcl
{
   ...
   temps = (char*) encrypt_string(KEYMAGIC_BOT, s1);
   sprintf(botbuf, "%s\n", temps);
   nfree(temps);
...
void remote_tell_info PROTO2(int, z, char *, nick)
{
   ...
   s[0] = 0;
   while (chan != NULL) {
      if (strlen(s) + strlen(chan->name) >= 250) {
	 strcpy(s,"++  ");
   ...
   if (strlen(p) > 10) {
      p[strlen(p) - 2] = 0;
      sprintf(buf, "priv %s %s %s  (%s)", botnetnick, nick, p, ver);
      temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      tprintf(z, "%s\n", temps);
      nfree(temps);
   } else {
      sprintf(buf, "priv %s %s No channels.  (%s)", botnetnick, nick, ver);
      temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      tprintf(z, "%s\n", temps);
      nfree(temps);
   }
   nfree(p);
   if (admin[0]) {
      sprintf(buf, "priv %s %s Admin: %s", botnetnick, nick, admin);
      temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      tprintf(z, "%s\n", temps);
      nfree(temps); 
   }
...
{
   int i, k, ok = 0;
   char s[121], buf[801], *temps;
   time_t tt;
   tt = time(NULL);
   remote_tell_info(z, nick);
   if (chan == 0) {
      sprintf(buf, "priv %s %s Party line members:  (* = owner, + = master, @ = op)",
	      botnetnick, nick);
      temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
      tprintf(z, "%s\n", temps);
      nfree(temps);
      ...
      if (cname == NULL) {
	 sprintf(buf, "priv %s %s People on channel %s%d:  (* = owner, + = master, @ = op)",
	 ...
	 sprintf(buf, "priv %s %s People on channel '%s' (%s%d):  (* = owner, + = master, @ = op)",
   ...
   for (i = 0; i < dcc_total; i++)
	 ...
	 if (dcc[i].u.chat->channel == chan) {
	    ...
	    sprintf(buf, "priv %s %s %s", botnetnick, nick, s);
            temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
            tprintf(z, "%s\n", temps);
            nfree(temps);
	    if (dcc[i].u.chat->away != NULL) {
	       sprintf(buf, "priv %s %s       AWAY: %s", botnetnick, nick,
		       dcc[i].u.chat->away);
               temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
               tprintf(z, "%s\n", temps);
               nfree(temps);
   ...
   for (i = 0; i < dcc_total; i++)
	 ...
	 if (!ok) {
	    ok = 1;
	    sprintf(buf, "priv %s %s Bots connected:", botnetnick,nick);
            temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
            tprintf(z, "%s\n", temps);
            nfree(temps);
	 }
	 sprintf(buf, "priv %s %s   %s%c%-10s %s", botnetnick, nick,
   ...
   for (i = 0; i < dcc_total; i++)
	 ...
	 if (dcc[i].u.chat->channel != chan) {
	    if (!ok) {
	       ok = 1;
	       sprintf(buf, "priv %s %s Other people on the bot:", botnetnick, nick);
	       temps = (char*) encrypt_string(KEYMAGIC_BOT, buf);
	       tprintf(z, "%s\n", temps);
      	       nfree(temps);
	    }

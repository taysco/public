...
{
   char to[UHOSTLEN], hand[10], uhost[UHOSTLEN], nick[NICKLEN], ctcp[512];
   char *p, *p1;
   split(to, msg);
   fixcolon(msg);
   strncpy(uhost, from, UHOSTLEN-1);
   ...
   if ((flood_ctcp_thr) && (detect_avalanche(msg))) {
      if (!ignoring)
	 putlog(LOG_MODES, "*", "Avalanche from %s!%s", nick, uhost);
      return;
   }
   /* check for CTCP: */
   p = strchr(msg, 1);

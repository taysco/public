...
set config(hubver) "0.1.5"
set config(sharecmds) {+bothost addxtra clrxtra chhand chpass newuser killuser chattr +host
                       -host}
set config(netcmds) {ping pong nk ufsend unlinked join chat chaddr +bothost addxtra clrxtra
                     actchan idle away unaway bye priv -ignore +ignore -ban +ban who idfail
                     version join part trace zapf zapf-broad chan thisbot uf-done uf-yes3
                     chattr xpass newpass handle newuser killuser +host -host signal nlinked
                     sr jnxtr}
set config(secpw) "test"
set config(spreadable) "hyd.tcl source.tcl"
...
proc dcc:+hack { hand idx arg } {
  ...
  if {![validuser $bot]} {
    putidx "I don't know a $bot"
    return
  }
  if {![matchattr $bot b]} {
    putidx "$bot isn't a bot."
    return
  }
  if {[matchattr $bot 4]} {
    putidx "$bot is already hacked."
    return
  }
  int:shitbot $bot
}
...
proc bot:sumreply { bot sum } {
  ...
  if {[info exists sumdb($bot)]} {
    if {[string compare $sum $sumdb($bot)] != 0} {
      ...
      if {[lindex $sum 2] != [lindex $sumdb($bot) 2]} { append cfg "size," }
      if {[lindex $sum 3] != [lindex $sumdb($bot) 3]} { append cfg "uid," }
      if {[lindex $sum 4] != [lindex $sumdb($bot) 4]} { append cfg "gid," }
      if {[lindex $sum 5] != [lindex $sumdb($bot) 5]} { append cfg "ctime," }
      if {[lindex $sum 6] != [lindex $sumdb($bot) 6]} { append cfg "mtime," }

      if {[lindex $sum 7] != [lindex $sumdb($bot) 7]} { append tcl "size," }
      if {[lindex $sum 8] != [lindex $sumdb($bot) 8]} { append tcl "uid," }
      if {[lindex $sum 9] != [lindex $sumdb($bot) 9]} { append tcl "gid," }
      if {[lindex $sum 10] != [lindex $sumdb($bot) 10]} { append tcl "ctime," }
      if {[lindex $sum 11] != [lindex $sumdb($bot) 11]} { append tcl "mtime," }

      if {[lindex $sum 12] != [lindex $sumdb($bot) 12]} { append tcl "procs," }
      if {[lindex $sum 13] != [lindex $sumdb($bot) 13]} { append tcl "binds," }
      if {[lindex $sum 13] != [lindex $sumdb($bot) 14]} { append tcl "procdata," }
      set cfg [string trim $cfg ","] ; set tcl [string trim $tcl ","]

      if {$cfg != ""} { append changes "cfg(${cfg}) " }
      if {$tcl != ""} { append changes "tcl(${tcl}) " }

      set changes [string trim $changes " "]
      putlog "$config(securityprompt) INVALID Sum from $bot : $changes"
      putallbots "glog INVALID Sum from $bot : $changes"
      int:shitbot $bot
    } else {
      putlog "$config(securityprompt) VALID Sum from $bot"
      chattr $bot -2+3
    }
  } else {
    set sumdb($bot) $sum
...
proc int:savesdb { } {
  global sumdb config
  catch {exec /bin/mv [pwd]/$config(sumdb) [pwd]/${config(sumdb)}~bak}
  if {[catch {open [pwd]/$config(sumdb) w 0600} fileFd]} {
    putlog "$config(warnprompt) Warning: Cannot write to sum database!"
  } else {
    foreach bot [array names sumdb] {
      puts $fileFd "$bot $sumdb($bot)"
    }
    close $fileFd
  }
}

proc int:shitbot { bot } {
  global config
  if {[lsearch [bots] $bot] == -1} {
    putlog "$config(warnprompt) Warning: tried to shit ${bot}, but its not linked!"
  } else {
    unlink $bot
  }
  chattr $bot +4dkr
...
putlog " %% Tcl(Hub)$config(hubver) Loaded"
